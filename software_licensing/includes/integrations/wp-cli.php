...
class EDD_SL_CLI extends EDD_CLI {
	...
	public function migrate_licenses( $args, $assoc_args ) {
		...
		if ( ! $force && $upgrade_completed ) {
			WP_CLI::error( __( 'The software licenses custom database migration has already been run. To do this anyway, use the --force argument.', 'templify_sl' ) );
		}
		...
		if ( ! empty( $total ) ) {
			...
			WP_CLI::line( __( 'License Migration complete.', 'templify_sl' ) );
			$new_count = $licenses_db->count();
			$old_count = $wpdb->get_col( "SELECT count(ID) FROM $wpdb->posts WHERE post_type ='edd_license'", 0 );
			WP_CLI::line( __( 'Old License Count: ', 'templify_sl' ) . $old_count[ 0 ] );
			WP_CLI::line( __( 'New License Count: ', 'templify_sl' ) . $new_count );

			update_option( 'edd_sl_version', preg_replace( '/[^0-9.].*/', '', EDD_SL_VERSION ) );

			delete_option( 'edd_sl_is_migrating_licenses' );

			WP_CLI::confirm( __( 'Remove legacy licenses?', 'templify_sl' ), $remove_args = array() );
			WP_CLI::line( __( 'Removing old license data.', 'templify_sl' ) );
			...
			WP_CLI::line( __( 'No licenses found.', 'templify_sl' ) );
			edd_set_upgrade_complete( 'migrate_licenses' );
			edd_set_upgrade_complete( 'migrate_license_parent_child' );
			edd_set_upgrade_complete( 'migrate_license_logs' );
			edd_set_upgrade_complete( 'remove_legacy_licenses' );
	...
	public function activate_licenses( $args, $assoc_args ) {
		...
		while ( $i <= $number ) {
			...
			WP_CLI::line( sprintf( __( 'URL %s added to %s', 'templify_sl' ), $site, $license->key ) );
			$i++;
		}
	}
