...
function edd_sl_show_errors() {
	...
	switch( $error ) {
		...
		case 'at_limit' :
			$message = __( 'This license is at its activation limit. Deactivate a site before adding a new one.', 'templify_sl' );
			break;

		case 'error_adding_site' :
			$message = __( 'There was an error adding your site. Please try again.', 'templify_sl' );
			break;
	}

	if( ! empty( $error ) ) {

		echo '<div class="edd_errors"><p class="edd_error"><strong>' . __( 'Error:', 'templify_sl' ) . '&nbsp;</strong>' . $message . '</p></div>';
