...
class EDD_SL_License {
		...
		if ( ! empty( $new_key ) ) {
			...
			if ( $updated ) {
				...
				$log_id = wp_insert_post(
					array(
						'post_title'   => __( 'LOG - License Key Regenerated: ', 'templify_sl' ) . $this->id,
	...
	public function get_display_status() {
		...
		switch ( $status ) {
			...
			case 'active' :
				$status = __( 'Active', 'templify_sl' );
				break;

			case 'inactive' :
				$status = __( 'Inactive', 'templify_sl' );
				break;

			case 'expired' :
				$status = __( 'Expired', 'templify_sl' );

				if ( edd_sl_renewals_allowed() ) {
					$renewal_link = $this->get_renewal_url();
					$status .= ', <a href="' . esc_url( $renewal_link ) . '" title="' . __( 'Renew this license', 'templify_sl' ) . '">' . __( 'renew now', 'templify_sl' ) . '</a>';
				}
				break;
	...
	public function license_limit() {
		...
		if ( $limit <= 0 ) {
			$limit = __( 'Unlimited', 'templify_sl' );
		}
	...
	public function license_term() {
		...
		if ( ! empty( $download_is_lifetime ) ) {
			$term = __( 'Lifetime', 'templify_sl' );
		} else {
			$exp_unit   = $download->get_expiration_unit_nicename( $price_id );
			$exp_length = $download->get_expiration_length( $price_id );

			if( empty( $exp_unit ) ) {
				$exp_unit = __( 'Years', 'templify_sl' );
			}
