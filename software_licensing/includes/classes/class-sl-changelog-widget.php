...
final class EDD_SL_Changelog_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			false,
			__( 'EDD Software Licensing Changelog', 'templify_sl' ),
			apply_filters(
				'edd_sl_changelog_widget_options',
				array(
					'classname'   => 'widget_sl_changelog',
					'description' => __( 'Display the changelog for a specific download.', 'templify_sl' )
				)
			)
		);
		$this->alt_option_name = 'widget_sl_changelog';
		$this->defaults = array(
			'title'        => __( 'Changelog', 'templify_sl' ),
	...
	public function form( $instance ) {
		...
		<p class="download-details-selector" <?php echo $display; ?>>
			...
			echo EDD()->html->product_dropdown( array(
				...
				'number'      => 15,
				'bundles'     => false,
				'placeholder' => sprintf( __( 'Choose a %s', 'templify_sl' ), edd_get_label_singular() ),
				'data'        => array(
					'search-type'        => 'download',
					'search-placeholder' => sprintf( __( 'Type to search all %s', 'templify_sl' ), edd_get_label_plural() )
				),
			) );
			?>
		</p>
