...
class EDD_Software_Licensing {
	...
	public function localization() {
		load_plugin_textdomain( 'templify_sl', false, dirname( plugin_basename( EDD_SL_PLUGIN_FILE ) ) . '/languages/' );
	}
...
>>>>>>> origin/master
	...
	*/
	...
	public function get_latest_version_remote( $data ) {
		...
		if ( empty( $data['products'] ) ) {
			$response['msg'] = __( 'No item provided', 'templify_sl' );
	...
	private function get_single_latest_version_remote( $data ) {
		...
		if ( ! $download ) {
			...
			if ( empty( $license ) || $check_by_name_first ) {
				...
				$response['msg'] = sprintf( __( 'Item name provided does not match a valid %s', 'templify_sl' ), edd_get_label_singular() );
			} else {
				/* translators: the singular download label */
				$response['msg'] = sprintf( __( 'License key provided does not match a valid %s', 'templify_sl' ), edd_get_label_singular() );
			}
		...
		if ( empty( $license ) ) {
			$message = __( 'No license key has been provided.', 'templify_sl' );
			...
			$response['msg'] = sprintf( __( 'License key is not valid for %s', 'templify_sl' ), $download_name );
	...
	*/
	...
	function log_license_activation( $license_id, $server_data ) {
		if ( $license = $this->get_license( $license_id ) ) {
			$license->add_log( __( 'LOG - License Activated: ', 'templify_sl' ) . $license_id, $server_data );
		}
	}
	...
	function log_license_deactivation( $license_id, $server_data ) {
		if ( $license = $this->get_license( $license_id ) ) {
			$license->add_log( __( 'LOG - License Deactivated: ', 'templify_sl' ) . $license_id, $server_data );
		}
	}
...
>>>>>>> origin/master
	...
	public function show_changelog() {
		...
		if( $changelog ) {
			...
			_e( 'No changelog found', 'templify_sl' );
		}
	...
	public function prevent_expired_downloads( $download_id, $email, $payment_id, $args ) {
		...
		if ( false === $can_download_response['success']  ) {
			$defaults = array(
				'message'  => __( 'You do not have a valid license for this download.', 'templify_sl' ),
				'title'    => __( 'No Valid License', 'templify_sl' ),
				'response' => 403,
			);
	...
	public function license_can_download( $download_id, $email, $payment_id, $args ) {
		...
		if ( count( $licenses ) === 1 ) {
			...
			if( 'expired' == $license->status ) {
				$can_download = array(
					'success'  => false,
					'message'  => __( 'Your license key for this purchase is expired. Renew your license key and you will be allowed to download your files again.', 'templify_sl' ),
					'title'    => __( 'Expired License', 'templify_sl' ),
					'response' => 401,
				);
			} elseif( 'disabled' === $license->status ) {
				$can_download = array(
					'success'  => false,
					'message'  => __( 'Your license key for this purchase has been revoked.', 'templify_sl' ),
					'title'    => __( 'Revoked License', 'templify_sl' ),
					'response' => 401,
				);
			}
