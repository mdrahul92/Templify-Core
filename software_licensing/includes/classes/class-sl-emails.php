...
class EDD_SL_Emails {
	...
	public function add_email_tag() {
		...
		edd_add_email_tag(
			'license_keys',
			__( 'Show all purchased licenses.', 'templify_sl' ),
			array( $this, 'licenses_tag' ),
			__( 'License Keys', 'templify_sl' )
		);
	...
	public function send_renewal_reminder( $license_id = 0, $notice_id = 0 ) {
		...
		if ( empty( $license_id ) ) {
			if ( $this->throw_exceptions ) {
				throw new \Exception( __( 'Reminder not sent: no license key provided.', 'templify_sl' ) );
			}
		...
		if ( ! edd_get_option( 'edd_sl_send_renewal_reminders', false ) ) {
			if ( $this->throw_exceptions ) {
				throw new \Exception( __( 'Reminder not sent: renewal reminders are not enabled.', 'templify_sl' ) );
			}
		...
		$exception_message = __( 'Reminder not sent: unexpected sending failure.', 'templify_sl' );

		if ( $license->is_lifetime ) {
			$exception_message = __( 'License never expires.', 'templify_sl' );
			$send              = false;
		}

		if ( $this->is_unsubscribed( $license ) ) {
			$exception_message = __( 'Reminder not sent: customer is not subscribed to reminder emails.', 'templify_sl' );
			$send              = false;
		}

		if ( 'disabled' === $license->status ) {
			$exception_message = __( 'Reminder not sent: this license key is disabled.', 'templify_sl' );
			$send              = false;
		}
		...
		$subject  = ! empty( $notice['subject'] ) ? $notice['subject'] : __( 'Your License Key is About to Expire', 'templify_sl' );
		...
		if ( $sent ) {
			$log_id = $license->add_log( __( 'LOG - Renewal Notice Sent', 'templify_sl' ), __( 'Sent via the send_renewal_reminder method.', 'templify_sl' ), 'renewal_notice' );
			...
			throw new \Exception( __( 'Reminder not sent: email failed to send.', 'templify_sl' ) );
		}
	...
	public function filter_reminder_template_tags( $text = '', $license_id = 0 ) {
		...
		if ( $license->expiration < $current_time ) {
			$time_diff = sprintf( __( 'expired %s ago', 'templify_sl' ), $time_diff );
		} else {
			$time_diff = sprintf( __( 'expires in %s', 'templify_sl' ), $time_diff );
		}
	...
	public function display_renewal_email_preview() {
		...
		if ( empty( $data['message'] ) ) {
			wp_die( esc_html__( 'The email message has no content.', 'templify_sl' ) );
		}
	...
	private function preview_reminder_template_tags( $text = '', $send_period = '+30 days' ) {
		...
		$text = str_replace( '{license_key}', __( 'Sample License Key', 'templify_sl' ), $text );
		$text = str_replace( '{product_name}', __( 'Sample Product Name', 'templify_sl' ), $text );
