...
class EDD_SL_Package_Download {
	...
	public function process_request() {
		...
		if ( false === $data['success'] ) {
			$message = ! empty( $data['message'] ) ? $data['message'] : __( 'An error has occurred, please contact support.', 'templify_sl' );
			wp_die( $message, __( 'Error', 'templify_sl' ), array( 'response' => 401 ) );
		}
	...
	public function parse_url() {
		...
		if ( count( $values ) !== 6 ) {
			return array(
				'success' => false,
				'message' => __( 'Invalid token supplied', 'templify_sl' ),
			);
		}
		...
		switch ( $license_status ) {
			...
			case 'expired':
				...
				$data = array(
					'success' => false,
					'message' => sprintf( __( 'Your license has expired, please <a href="%s" title="Renew your license">renew it</a> to install this update.', 'templify_sl' ), $renewal_link ),
				);
				break;
			...
			case 'site_inactive':
				$data = array(
					'success' => false,
					'message' => __( 'Your license has not been activated for this domain, please activate it first.', 'templify_sl' ),
				);
				break;
			...
			case 'disabled':
				$data = array(
					'success' => false,
					'message' => __( 'Your license has been disabled.', 'templify_sl' ),
				);
				break;
			...
			default:
				$data = array(
					'success' => false,
					'message' => __( 'Your license could not be validated.', 'templify_sl' ),
				);
				break;
		}
		...
		if ( ! hash_equals( $computed_hash, $values[3] ) ) {
			return array(
				'success' => false,
				'message' => __( 'Provided hash does not validate.', 'templify_sl' ),
			);
		}
	...
	public function process_package_download() {
		...
		if ( isset( $_GET['key'] ) && isset( $_GET['id'] ) && isset( $_GET['license'] ) && isset( $_GET['expires'] ) ) {
			...
			if ( current_time( 'timestamp' ) > $expires ) {
				wp_die( __( 'Your download link has expired', 'templify_sl' ), __( 'Error', 'templify_sl' ), array( 'response' => 401 ) );
			}

			if ( empty( $license ) ) {
				wp_die( __( 'No license key provided', 'templify_sl' ), __( 'Error', 'templify_sl' ), array( 'response' => 401 ) );
			}

			if (  ! edd_software_licensing()->is_download_id_valid_for_license( $id, $license ) ) {
				wp_die( __( 'Invalid license supplied', 'templify_sl' ), __( 'Error', 'templify_sl' ), array( 'response' => 401 ) );
			}

			$requested_file = $this->get_download_package( $id, $license, $hash, $expires, $download_beta );
			if ( ! $requested_file ) {
				wp_die( esc_html__( 'No download package available', 'templify_sl' ), esc_html__( 'Error', 'templify_sl' ), array( 'response' => 401 ) );
			}
			...
			wp_die( __( 'You do not have permission to download this file', 'templify_sl' ), __( 'Error', 'templify_sl' ), array( 'response' => 401 ) );
		}
	...
	public function get_download_package( $download_id, $license_key, $hash, $expires = 0, $download_beta = false ) {
		...
		if ( ! empty( $hash ) && ! hash_equals( $computed_hash, $hash ) ) {
			wp_die( __( 'You do not have permission to download this file. An invalid hash was provided.', 'templify_sl' ), __( 'Error', 'templify_sl' ), array( 'response' => 401 ) );
		}
