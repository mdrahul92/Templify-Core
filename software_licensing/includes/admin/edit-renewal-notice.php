...
if ( ! isset( $_GET['notice'] ) || ! is_numeric( $_GET['notice'] ) ) {
	//wp_die( __( 'Something went wrong.', 'templify_sl' ), __( 'Error', 'templify_sl' ) );
}
?>
...
?>
	<h1><?php esc_html_e( 'Edit Renewal Notice', 'templify_sl' ); ?></h1>
	<?php
	...
	?>
	<a href="<?php echo esc_url( $settings_page_url ); ?>"><?php esc_html_e( 'Return to Settings', 'templify_sl' ); ?></a> | <a href="<?php echo esc_url( wp_nonce_url( $preview_url ) ); ?>" class="edd-sl-preview-renewal-notice" data-key="<?php echo esc_attr( $notice_id ); ?>" target="_blank"><?php esc_html_e( 'Preview', 'templify_sl' ); ?></a>
	<form id="edd-edit-renewal-notice" action="" method="post">
		<table class="form-table">
			<tbody>
				<tr>
					<th scope="row" valign="top">
						<label for="edd-notice-subject"><?php esc_html_e( 'Email Subject', 'templify_sl' ); ?></label>
					</th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'The subject line of the renewal notice email', 'templify_sl' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row" valign="top">
						<label for="edd-notice-period"><?php esc_html_e( 'Email Period', 'templify_sl' ); ?></label>
					</th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'When should this email be sent?', 'templify_sl' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row" valign="top">
						<label for="edd-notice-message"><?php esc_html_e( 'Email Message', 'templify_sl' ); ?></label>
					</th>
					<td>
						<?php wp_editor( wp_kses_post( wptexturize( $notice['message'] ) ), 'message', array( 'textarea_name' => 'message' ) ); ?>
						<p class="description"><?php esc_html_e( 'The email message to be sent with the renewal notice. The following template tags can be used in the message:', 'templify_sl' ); ?></p>
		...
		<p class="submit">
			...
			<input type="submit" value="<?php esc_attr_e( 'Update Renewal Notice', 'templify_sl' ); ?>" class="button-primary"/>
		</p>
	</form>
</div>
