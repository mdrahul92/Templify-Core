...
function edd_sl_register_upgrades_page() {
	add_submenu_page( 'edd-sl-upgrades-page', __( 'EDD SL Upgrades', 'templify_sl' ), __( 'EDD Upgrades', 'templify_sl' ), 'manage_options', 'edd-sl-upgrades', 'edd_sl_upgrades_screen' );
...
function edd_sl_hide_upgrades_page() {
	remove_submenu_page( 'edit.php?post_type=download', 'edd-sl-upgrades-page' );
}
...
function edd_sl_upgrades_screen() {
	...
	?>
	<div class="wrap">
		<h2><?php _e( 'Software Licensing - Upgrades', 'templify_sl' ); ?></h2>
		<?php
...
function edd_sl_show_upgrade_notice() {
	...
	if ( ! $licenses_migrated ) {
		...
		if ( ! $has_licenses ) {
			...
			printf(
				...
				esc_html__( 'Your Software Licensing database needs an upgrade. Once you have completed the migration to Easy Digital Downloads 3.0, the upgrade prompt will appear here.', 'templify_sl' )
			);
		} else {
			printf(
				'<div class="updated">' .
				'<p>' .
				__( 'Easy Digital Downloads - Software Licensing needs to upgrade the licenses database, click <a href="%s">here</a> to start the upgrade. <a href="#" onClick="jQuery(this).parent().next(\'p\').slideToggle()">Learn more about this upgrade</a>.', 'templify_sl' ) .
				'</p>' .
				'<p style="display: none;">' .
				__( '<strong>About this upgrade:</strong><br />This is a <strong><em>mandatory</em></strong> update that will migrate all licenses and their meta data to a new custom database table. This upgrade should provide better performance and scalability.', 'templify_sl' ) .
				'<br /><br />' .
				__( '<strong>Please back up your database before starting this upgrade.</strong> This upgrade routine will make irreversible changes to the database.', 'templify_sl' ) .
				'<br /><br />' .
				__( '<strong>Advanced User?</strong><br />This upgrade can also be run via WP-CLI with the following command:<br /><code>wp edd-sl migrate_licenses</code>', 'templify_sl' ) .
				'<br /><br />' .
				__( 'For large sites, this is the recommended method of upgrading.', 'templify_sl' ) .
				'</p>' .
				'</div>',
	...
	if ( $licenses_migrated && ( ! $bundle_licenses_updated || ! $license_logs_updated ) ) {
		...
		printf(
			'<div class="error">' .
			'<p>' .
			__( 'Easy Digital Downloads - Software Licensing still needs to complete the upgrade to the licenses database, click <a href="%s">here</a> to continue the upgrade.', 'templify_sl' ) .
			'</p>' .
			'</div>',
			esc_url( admin_url( 'index.php?page=edd-sl-upgrades&edd-upgrade=licenses_migration' ) )
		);
	...
	if ( ( $licenses_migrated && $bundle_licenses_updated && $license_logs_updated ) && ! $removed_legacy_licenses ) {
		printf(
			'<div class="updated">' .
			'<p>' .
			__( 'Easy Digital Downloads - Software Licensing has <strong>finished upgrading the licenses database</strong>. The final step is to <a href="%s">remove the legacy data</a>. <a href="#" onClick="jQuery(this).parent().next(\'p\').slideToggle()">Learn more about this process</a>.', 'templify_sl' ) .
			'</p>' .
			'<p style="display: none;">' .
			__( '<strong>Removing legacy data:</strong><br />All licenses have been migrated to their own custom table. Now all old data needs to be removed.', 'templify_sl' ) .
			'<br /><br />' .
			__( '<strong>If you have not already, back up your database</strong> as this upgrade routine will be making changes to the database that are not reversible.', 'templify_sl' ) .
			'</p>' .
			'</div>',
			esc_url( admin_url( 'index.php?page=edd-sl-upgrades&edd-upgrade=licenses_migration' ) )
		);
	...
	if ( $licenses_migrated && ! $fix_no_url_check_activation_counts ) {
		...
		if ( false === $url_checking_disabled ) {
			...
			printf(
				'<div class="updated">' .
				'<p>' .
				__( 'Easy Digital Downloads - Software Licensing needs to <a href="%s">update your license activation records</a>.', 'templify_sl' ) .
				'</p>' .
				'</div>',
...
function edd_sl_render_licenses_migration() {
	...
	if ( $migration_complete && $removal_complete && $logs_complete && $removal_complete ) : ?>
		<div id="edd-sl-migration-complete" class="notice notice-success">
			<p>
				<?php _e( '<strong>Migration complete:</strong> You have already completed the migration of licenses to custom database tables.', 'templify_sl' ); ?>
			</p>
		</div>
		<?php return; ?>
	<?php endif; ?>
	...
	<div id="edd-sl-migration-ready" class="notice notice-success" style="display: none;">
		<p>
			<?php _e( '<strong>Database Upgrade Complete:</strong> All database upgrades have been completed. We recommended you now verify your store\'s operations are functioning as expected.', 'templify_sl' ); ?>
			<br /><br />
			<?php _e( 'You may now leave this page.', 'templify_sl' ); ?>
		</p>
	</div>
	<?php
	...
	<div id="edd-sl-migration-nav-warn" class="notice notice-info">
		<p>
			<?php _e( '<strong>Important:</strong> Please leave this screen open and do not navigate away until the process completes.', 'templify_sl' ); ?>
		</p>
	</div>
	...
	<div class="metabox-holder">
		<div class="postbox">
			<h2 class="hndle">
				<span><?php printf( __( 'Step %d: Upgrade Licenses Database', 'templify_sl' ), $step ); ?></span>
				 <span class="dashicons dashicons-yes"></span>
			</h2>
			<div class="inside migrate-licenses-control">
				<p>
					<?php _e( 'This will upgrade the licenses database for improved performance and reliability.', 'templify_sl' ); ?>
				</p>
					...
					<span class="step-instructions-wrapper">
						...
						<?php if ( ! $removal_complete ) : ?>
							<span class="edd-sl-migration allowed" style="<?php echo ! $migration_complete ? '' : 'display: none'; ?>">
								<input type="submit" id="migrate-licenses-submit" value="<?php _e( 'Upgrade Database', 'templify_sl' ); ?>" class="button-primary"/>
							</span>

							<span class="edd-sl-migration unavailable" style="<?php echo $migration_complete ? '' : 'display: none'; ?>">
								<input type="submit" disabled="disabled" id="migrate-licenses-submit" value="<?php _e( 'Upgrade Database', 'templify_sl' ); ?>" class="button-secondary"/>
								&mdash; <?php _e( 'Your licenses database has been upgraded.', 'templify_sl' ); ?>
							</span>
						<?php else: ?>
							<input type="submit" disabled="disabled" id="migrate-licenses-submit" value="<?php _e( 'Upgrade Database', 'templify_sl' ); ?>" class="button-secondary"/>
							&mdash; <?php _e( 'Legacy data has already been removed, migration is not possible at this time.', 'templify_sl' ); ?>
						<?php endif; ?>
	...
	<div class="metabox-holder">
		<div class="postbox">
			<h2 class="hndle">
				<span><?php printf( __( 'Step %d: Update Bundled Licenses', 'templify_sl' ), $step ); ?></span>
				 <span class="dashicons dashicons-yes"></span>
			</h2>
			<div class="inside migrate-licenses-control">
				<p>
					<?php _e( 'This restores child licenses with their new parent license IDs.', 'templify_sl' ); ?>
				</p>
				...
				<span class="step-instructions-wrapper">
					...
					<?php if ( ! $relationships_complete ) : ?>
						<span class="edd-sl-migration allowed" style="<?php echo $migration_complete ? '' : 'display: none'; ?>">
							<input type="submit" id="migrate-bundles-submit" value="<?php _e( 'Update Bundles', 'templify_sl' ); ?>" class="button-primary"/>
						</span>

						<span class="edd-sl-migration unavailable" style="<?php echo ! $migration_complete ? '' : 'display: none'; ?>">
							<input type="submit" disabled="disabled" id="migrate-bundles-submit" value="<?php _e( 'Update Bundles', 'templify_sl' ); ?>" class="button-secondary"/>
							&mdash; <?php _e( 'Please complete the previous step before updating bundled licenses.', 'templify_sl' ); ?>
						</span>
					<?php else : ?>
						<input type="submit" disabled="disabled" id="migrate-bundles-submit" value="<?php _e( 'Update Bundles', 'templify_sl' ); ?>" class="button-secondary"/>
						&mdash; <?php _e( 'Bundled licenses already updated.', 'templify_sl' ); ?>
					<?php endif; ?>
	...
	<div class="metabox-holder">
		<div class="postbox">
			<h2 class="hndle">
				<span><?php printf( __( 'Step %d: Update License Logs', 'templify_sl' ), $step ); ?></span>
				 <span class="dashicons dashicons-yes"></span>
			</h2>
			<div class="inside migrate-licenses-control">
				<p>
					<?php _e( 'This updates the license logs with the new license data.', 'templify_sl' ); ?>
				</p>
				...
				<span class="step-instructions-wrapper">
					...
					<?php if ( ! $logs_complete ) : ?>
						<span class="edd-sl-migration allowed" style="<?php echo $migration_complete ? '' : 'display: none'; ?>">
							<input type="submit" id="migrate-logs-submit" value="<?php _e( 'Update License Logs', 'templify_sl' ); ?>" class="button-primary"/>
						</span>

						<span class="edd-sl-migration unavailable" style="<?php echo ! $migration_complete ? '' : 'display: none'; ?>">
							<input type="submit" disabled="disabled" id="migrate-logs-submit" value="<?php _e( 'Update License Logs', 'templify_sl' ); ?>" class="button-secondary"/>
							&mdash; <?php _e( 'Please complete the previous steps before updating the license logs.', 'templify_sl' ); ?>
						</span>
					<?php else: ?>
						<input type="submit" disabled="disabled" id="migrate-logs-submit" value="<?php _e( 'Update License Logs', 'templify_sl' ); ?>" class="button-secondary"/>
						&mdash; <?php _e( 'License logs have already been updated.', 'templify_sl' ); ?>
					<?php endif; ?>
	...
	<div class="metabox-holder">
		<div class="postbox">
			<h2 class="hndle">
				<span><?php printf( __( 'Step %d: Remove Legacy Data', 'templify_sl' ), $step ); ?></span>
				 <span class="dashicons dashicons-yes"></span>
			</h2>
			<div class="inside migrate-licenses-control">
				<p>
					<?php _e( 'This will remove all legacy license data.', 'templify_sl' ); ?>
				</p>
				<p>
					<?php _e( '<strong>Important:</strong> Please be sure to back up your database prior to completing this step. The actions taken during this step are irreversible.', 'templify_sl' ); ?>
				</p>
					...
					<span class="step-instructions-wrapper">
						...
						<?php if ( ! $removal_complete ) : ?>
							<span class="edd-sl-migration allowed">
								<input type="submit" id="remove-legacy-licenses-submit" value="<?php _e( 'Remove Legacy Data', 'templify_sl' ); ?>" class="button-primary"/>
							</span>
						<?php elseif ( $removal_complete ): ?>
							<input type="submit" disabled="disabled" id="remove-legacy-licenses-submit" value="<?php _e( 'Remove Legacy Data', 'templify_sl' ); ?>" class="button-secondary"/>
							&mdash; <?php _e( 'Legacy data has already been removed.', 'templify_sl' ); ?>
						<?php endif; ?>
...
function edd_sl_render_fix_no_url_check_activation_counts() {
	if ( edd_has_upgrade_completed( 'fix_no_url_check_activation_counts' ) ) {
		...
		<div class="notice notice-success">
			<p>
				<?php _e( 'This upgrade has already been completed.', 'templify_sl' ); ?>
			</p>
		</div>
		<?php
		return;
	...
	<div class="metabox-holder">
		<div class="postbox">
			<h2 class="hndle">
				<span><?php _e( 'Update License Activation Records', 'templify_sl' ); ?></span>
				<span class="dashicons dashicons-yes"></span>
			</h2>
			<div class="inside update-licenses-control">
				<p>
					<?php _e( 'This will check your license activation counts, and use the activation logs to correct any discrepancies in the activation counts.', 'templify_sl' ); ?>
				</p>
				<form method="post" id="edd-sl-fix-license-activation-form" class="edd-export-form edd-import-export-form">
					<span class="step-instructions-wrapper">
						...
						<span class="edd-sl-fix-license-activations allowed">
							<input type="submit" id="fix-license-activation-submit" value="<?php _e( 'Update Activation Counts', 'templify_sl' ); ?>" class="button-primary"/>
						</span>
