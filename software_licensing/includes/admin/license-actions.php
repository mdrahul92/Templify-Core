...
function edd_sl_add_licenses_link() {
	...
	$edd_sl_licenses_page = add_submenu_page( 'edit.php?post_type=download', __( 'Easy Digital Download Licenses', 'templify_sl' ), __( 'Licenses', 'templify_sl' ), 'view_licenses', 'edd-licenses', 'edd_sl_licenses_page' );
	$edd_sl_licenses_page = add_submenu_page( 'edit.php?post_type=download', __( 'License Renewal Notice', 'templify_sl' ), __( 'License Renewal Notice', 'templify_sl' ), 'manage_shop_settings', 'edd-license-renewal-notice', 'edd_sl_license_renewal_notice_edit' );
...
function edd_sl_admin_settings_page_title( $title, $current_page ) {
	...
	if ( 'add-renewal-notice' === $page ) {
		return __( 'Add License Renewal Notice', 'templify_sl' );
	}

	return __( 'Edit License Renewal Notice', 'templify_sl' );
}
...
function edd_sl_process_regenerate_license_key() {
	...
	if ( ! current_user_can( 'manage_licenses' ) ) {
		$response = array(
			'success' => false,
			'message' => __( 'You do not have permission to manage this license.', 'templify_sl' ),
		);
	...
	if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'edd-sl-regenerate-license' ) ) {
		$response = array(
			'success' => false,
			'message' => __( 'Nonce validation failed.', 'templify_sl' ),
		);
	...
	if ( ! isset( $_POST['license_id'] ) || ! is_numeric( $_POST['license_id'] ) ) {
		$response = array(
			'success' => false,
			'message' => __( 'No license provided.', 'templify_sl' ),
		);
	...
	if ( false === $license ) {
		$response = array(
			'success' => false,
			'message' => __( 'Invalid license supplied.', 'templify_sl' ),
		);
	...
	if ( $generated ) {
		$response = array(
			'success' => true,
			'message' => __( 'License key generated successfully.', 'templify_sl' ),
			'key'     => $license->key,
		);
		...
		$response = array(
			'success' => false,
			'message' => __( 'Failed to generate a new license key.', 'templify_sl' ),
		);
...
function edd_sl_process_license_exp_update() {
	...
	if ( ! wp_verify_nonce( $_POST['edd-sl-update-license-nonce'], 'edd-sl-update-license' ) ) {
		wp_die( __( 'Nonce verification failed', 'templify_sl' ), __( 'Error', 'templify_sl' ), array( 'response' => 403 ) );
	}
	...
	if ( isset( $_POST['price_id'] ) && $license->get_download()->has_variable_prices() ) {
		...
		if ( (int) $price_id !== (int) $license->price_id ) {
			...
			$log_id = wp_insert_post(
				array(
					'post_title'   => sprintf( __( 'License Price ID modified to %d', 'templify_sl' ), $price_id ),
			...
			$payment->add_note( sprintf( __( 'License Price ID for %s changed from %d to %d.', 'templify_sl' ), $license->get_name( false ), $old_price_id, $price_id ) );
		}
	}
...
function edd_sl_send_renewal_notice() {
	...
	if ( ! empty( $_POST['license_id'] ) ) {
		...
		if ( ! current_user_can( 'manage_licenses', $payment_id ) ) {
			wp_die( __( 'You do not have permission to edit this license', 'templify_sl' ), __( 'Error', 'templify_sl' ), array( 'response' => 403 ) );
		}
...
function edd_sl_delete_license( $args ) {
	...
	if ( ! wp_verify_nonce( $nonce, 'delete-license' ) ) {
		wp_die( __( 'Cheatin\' eh?!', 'templify_sl' ) );
	}

	if ( ! $confirm ) {
		edd_set_error( 'license-delete-no-confirm', __( 'Please confirm you want to delete this license', 'templify_sl' ) );
	}
...
function edd_sl_log_generated_license( $license_id, $d_id, $payment_id, $type ) {
	...
	if ( $license ) {
		$license->add_log(
			sprintf( __( 'Missing License Generated: %s', 'templify_sl' ), $license_id ),
			array(
				'HTTP_USER_AGENT' => $_SERVER['HTTP_USER_AGENT'],
				'REMOTE_ADDR'     => $_SERVER['REMOTE_ADDR'],
				'REQUEST_TIME'    => $_SERVER['REQUEST_TIME']
...
function edd_sl_ajax_get_license_logs() {
	...
	if ( $logs ) {
		...
		$html = '<p>' . __( 'This license has no log entries', 'templify_sl' );
	}
...
function edd_sl_toggle_license_subscription( $args ) {
	// check permissions.
	if ( ! current_user_can( 'manage_licenses' ) ) {
		wp_die( esc_html__( 'You do not have permission to edit this license.', 'templify_sl' ), esc_html__( 'Error', 'templify_sl' ), array( 'response' => 403 ) );
	}

	// verify nonce.
	if ( empty( $args['_wpnonce'] ) || ! wp_verify_nonce( $args['_wpnonce'], 'edd_sl_update_email_notifications' ) ) {
		wp_die( esc_html__( 'You do not have permission to perform this action.', 'templify_sl' ), esc_html__( 'Error', 'templify_sl' ), array( 'response' => 403 ) );
	}

	// check the license id.
	if ( ! isset( $args['license_id'] ) ) {
		wp_die( esc_html__( 'No license ID supplied or invalid key provided.', 'templify_sl' ), esc_html__( 'Error', 'templify_sl' ), array( 'response' => 400 ) );
	}

	// get the license.
	$license = edd_software_licensing()->get_license( $args['license_id'] );
	if ( false === $license ) {
		wp_die( esc_html__( 'Invalid license ID supplied or invalid key provided.', 'templify_sl' ), esc_html__( 'Error', 'templify_sl' ), array( 'response' => 400 ) );
	}
