...
function edd_sl_customer_tab( $tabs ) {
	...
	if ( $licenses ) {
		...
		$tabs['licenses'] = array( 'dashicon' => 'dashicons-lock', 'title' => __( 'License Keys', 'templify_sl' ) );
...
function edd_sl_customer_licenses_view( $customer ) {
	...
	<?php if ( $licenses ) :
		...
		<div id="customer-tables-wrapper" class="customer-section">
			<h3><?php esc_html_e( 'License Keys', 'templify_sl' ); ?></h3>

			<table class="wp-list-table widefat striped downloads">
				<thead>
					<tr>
						<th><?php echo edd_get_label_singular(); ?></th>
						<th><?php esc_html_e( 'License Key', 'templify_sl' ); ?></th>
						<th><?php esc_html_e( 'Status', 'templify_sl' ); ?></th>
						<th width="120px"><?php esc_html_e( 'Actions', 'templify_sl' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php if ( ! empty( $all_license_keys ) ) : ?>
						<?php foreach ( $all_license_keys as $license ) : ?>
							<tr>
								...
								<td>
									<a title="<?php esc_attr_e( 'View', 'templify_sl' ); ?>" href="<?php echo esc_url( admin_url( 'edit.php?post_type=download&page=edd-licenses&view=overview&license_id=' . $license->ID ) ); ?>">
										<?php esc_html_e( 'View', 'templify_sl' ); ?>
									</a>
								</td>
							</tr>
						<?php endforeach; ?>
					<?php else: ?>
						<tr><td colspan="2"><?php esc_html_e( 'No license keys found', 'templify_sl' ); ?></td></tr>
					<?php endif; ?>
				</tbody>
			</table>
