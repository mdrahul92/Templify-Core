...
class EDD_SL_License_Activation_Count_Fix extends EDD_Batch_Export {
		...
		if ( $step_items ) {
			...
			foreach ( $step_items as $item ) {
				...
				foreach( $license->get_logs() as $log ) {
					if ( false !== strpos( $log->post_title, __( 'LOG - License Activated: ', 'templify_sl' ) . $item ) ) {
						$activations++;
					} else if ( false !== strpos( $log->post_title, __( 'LOG - License Deactivated: ', 'templify_sl' ) . $item ) ) {
						$deactivations++;
					}
				}
				...
				if ( ! empty( $total_activations ) ) {
					...
					$license->add_log( __( 'License activation count updated via update routine.', 'templify_sl' ), __( 'Activation count updated based off activation/deactivation logs via update routine', 'templify_sl' ) );
				} else {
	...
	public function process_step() {
		if ( ! $this->can_export() ) {
			wp_die( __( 'You do not have permission to run this update.', 'templify_sl' ), __( 'Error', 'templify_sl' ), array( 'response' => 403 ) );
		}
		...
		if ( $had_data ) {
			...
			$this->message = __( 'License activations update complete.', 'templify_sl' );
