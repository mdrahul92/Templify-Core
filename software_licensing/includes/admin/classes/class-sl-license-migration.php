...
class EDD_SL_License_Migration extends EDD_Batch_Export {
		...
		if ( $step_items ) {
			...
			foreach ( $step_items as $item ) {
				...
				if ( empty( $post_meta['_edd_sl_key'] ) ) {
					edd_debug_log( sprintf( __( 'Legacy License ID %d failed: Missing license key', 'templify_sl'), $license_post->ID ) );
					continue;
				}
				...
				if ( ! empty( $new_license_id ) ) {
					...
					edd_debug_log( sprintf( __( 'Legacy License ID %d failed: Failed to insert new license record', 'templify_sl'), $license_post->ID ) );
				}
	...
	public function process_step() {
		if ( ! $this->can_export() ) {
			wp_die( __( 'You do not have permission to migrate licenses.', 'templify_sl' ), __( 'Error', 'templify_sl' ), array( 'response' => 403 ) );
		}
		...
		if( $had_data ) {
			...
			$this->message = sprintf( __( 'Licenses database upgrade complete. Migrated <span class="edd-sl-new-count">%d</span> of <span class="edd-sl-old-count">%d</span> licenses.', 'templify_sl' ), $new_count, $old_count );
