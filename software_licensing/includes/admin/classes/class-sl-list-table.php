...
class EDD_SL_List_Table extends WP_List_Table {
		...
		switch ( $column_name ) {
			...
			case 'key':
				...
				if ( 'disabled' === $item['status'] ) {
					echo ' <em>(' . esc_html__( 'disabled', 'templify_sl' ) . ')</em>';
				}
			...
			case 'user':
				...
				echo '<a href="' . esc_url( $licenses_link ) . '">' . esc_html__( 'View other licenses', 'templify_sl' ) . '</a>';
				break;

			case 'limit':
				$limit = $item['activation_limit'] > 0 ? intval( $item['activation_limit'] ) : __( 'Unlimited', 'templify_sl' );
				echo esc_html( $item['activation_count'] ) . ' / ' . esc_html( $limit );
				break;

			case 'expires':
				if ( $item['is_lifetime'] ) {
					esc_html_e( 'Lifetime', 'templify_sl' );
				} else {
			...
			case 'purchased':
				$purchased = __( 'Order', 'templify_sl' ) . ' ' . $item['order_number'] . '<br />' . $item['purchased'];
	...
	public function column_cb( $item ) {
		return sprintf(
			...
			sprintf( __( 'Select License Key %s', 'templify_sl' ), $item['key'] )
		);
	}
	...
	public function get_columns() {
		return apply_filters(
			...
			array(
				...
				'key'       => __( 'Key', 'templify_sl' ),
				'title'     => __( 'Product', 'templify_sl' ),
				'user'      => __( 'Customer', 'templify_sl' ),
				'limit'     => __( 'Activations', 'templify_sl' ),
				'expires'   => __( 'Expires', 'templify_sl' ),
				'purchased' => __( 'Purchased', 'templify_sl' ),
			)
		);
	}
	...
	function get_bulk_actions() {
		...
		$actions = array(
			'enable'         => __( 'Enable', 'templify_sl' ),
			'disable'        => __( 'Disable', 'templify_sl' ),
			'renewal_notice' => __( 'Send Renewal Notice', 'templify_sl' ),
			'renew'          => __( 'Renew', 'templify_sl' ),
			'delete'         => __( 'Delete', 'templify_sl' )
		);
	...
	private function get_licenses_data_args( $license, $parent = false ) {
		...
		$date         = esc_html__( 'Unknown Date', 'templify_sl' );
