...
class EDD_SL_Retroactive_Licensing {
	...
	*/
	public function tool_box() {
		...
		<div class="postbox edd-sl-retroactive-licensing">
			<h3 id="edd-sl-retroactive-processor"><?php esc_html_e( 'Software Licensing - Retroactive Licensing Processor', 'templify_sl' ); ?></h3>
			<div class="inside">
				<?php
				if ( ! current_user_can( 'manage_licenses' ) ) {
					...
					?>
					<p>
						<?php _e( 'You do not have permission to use the Retroactive Licensing Processor.', 'templify_sl' ); ?>
					</p>
					<?php
				} else {
					...
					if ( ! empty( $_POST[ 'edd-retroactive-licensing' ] ) || ! empty( $_REQUEST['posts'] ) ) {
						...
						if ( ! $count ) {
							?>
							<h3><span><?php _e( 'All Done', 'templify_sl' ); ?></span></h3>
							<h4><?php _e( 'All Done', 'templify_sl' ); ?></h4>
							<p><?php _e( 'No purchases needing licenses found.', 'templify_sl' ); ?></p>
							<?php
							return;
						}
						...
						?>
						<p><?php _e( 'Use this tool to provision licenses for unlicensed Easy Digital Downloads products.', 'templify_sl' ); ?></p>
						<p><?php _e( 'This processing is not reversible. Backup your database beforehand or be prepared to revert each transformed post manually.', 'templify_sl' ); ?></p>
						<form method="post" action="">
							<?php wp_nonce_field( 'edd-retroactive-licensing' ); ?>
							<p>
								<span id="sl-retro-type-wrapper">
									<select name="sl_retro_type" id="sl-retro-type">
										<option value="all"><?php printf( __( 'All %s', 'templify_sl' ), edd_get_label_plural() ); ?></option>
										<option value="single"><?php printf( __( 'Single %s', 'templify_sl' ), edd_get_label_singular() ); ?></option>
									</select>
								<span>
								...
								<input type="submit" class="button hide-if-no-js" name="edd-retroactive-licensing" id="edd-retroactive-licensing" value="<?php _e( 'Generate License Keys for Past Purchases', 'templify_sl' ) ?>" />
								<?php wp_nonce_field( 'edd_sl_retroactive', 'edd_sl_retroactive' ); ?>
							</p>
							<noscript><p><em><?php _e( 'You must enable Javascript in order to proceed!', 'templify_sl' ) ?></em></p></noscript>
						</form>
						<?php
					}
				}
	...
	*/
	public function show_status( $count, $posts, $download_id ) {
		echo '<p>' . esc_html__( 'Please be patient while this script runs. This can take a while, up to a minute per payment. Do not navigate away from this page until this script is done or the licensing will not be completed. You will be notified via this page when the licensing is completed.', 'templify_sl' ) . '</p>';

		$text_goback = ( ! empty( $_GET['goback'] ) ) ? sprintf( __( 'To go back to the previous page, <a href="%s">click here</a>.', 'templify_sl' ), 'javascript:history.go(-1)' ) : '';

		$text_failures = sprintf( __( 'All done! %1$s posts were successfully processed in %2$s seconds and there were %3$s failures. To try importing the failed posts again, <a href="%4$s">click here</a>. %5$s', 'templify_sl' ), "' + rt_successes + '", "' + rt_totaltime + '", "' + rt_errors + '", esc_url( wp_nonce_url( admin_url( 'edit.php?post_type=download&?page=edd-retroactive-licensing&goback=1' ) ) . '&posts=' ) . "' + rt_failedlist + '", $text_goback );

		$text_nofailures = sprintf( esc_html__( 'All done! %1$s posts were successfully processed in %2$s seconds and there were no failures. %3$s', 'templify_sl' ), "' + rt_successes + '", "' + rt_totaltime + '", $text_goback );
?>
		...
		<h3 class="title"><?php _e( 'Status', 'templify_sl' ) ?></h3>

		<div><input type="button" class="button hide-if-no-js" name="wpsposts-stop" id="wpsposts-stop" value="<?php _e( 'Abort Licensing Posts', 'templify_sl' ) ?>" /></div>

		<p>
			<?php printf( esc_html__( 'Total Payments: %s', 'templify_sl' ), $count ); ?><br />
			<?php printf( esc_html__( 'Payments Processed: %s', 'templify_sl' ), '<span id="wpsposts-debug-successcount">0</span>' ); ?><br />
			<?php printf( esc_html__( 'License Failures: %s', 'templify_sl' ), '<span id="wpsposts-debug-failurecount">0</span>' ); ?>
		</p>
		...
		// <![CDATA[
			jQuery(document).ready(function($){
				...
				$( "#wpsposts-stop" ).click(function() {
					rt_continue = false;
					$( '#wpsposts-stop' ).val( "<?php echo esc_html__( 'Stopping, please wait a moment.', 'templify_sl' ); ?>" );
				});
...
<?php
	...
	*/
	public function edd_sl_process_retroactive_post() {
		...
		if ( ! current_user_can( 'manage_licenses' ) ) {
			die( json_encode( array( 'error' => __( 'Failed Licensing: You do not have permission to perform this action.', 'templify_sl' ) ) ) );
		}
		...
		if ( ! $payment ) {
			...
			die( wp_json_encode( array( 'error' => sprintf( esc_html__( 'Failed Licensing: %s is incorrect post type.', 'templify_sl' ), esc_html( $order_number ) ) ) ) );
		}
		...
		if ( ! empty( $number_generated ) && is_numeric( $number_generated ) ) {
			...
			$message = _n( '%1$s License key for Order <a href="%2$s" target="_blank">%3$s</a> was successfully processed.', '%1$s License keys for Order <a href="%2$s" target="_blank">%3$s</a> were successfully processed.', $number_generated, 'templify_sl' );
			...
			die( json_encode( array( 'success' => sprintf( __( 'Order <a href="%1$s" target="_blank">%2$s</a> processed. No licenses needed to be processed.', 'templify_sl' ), self::get_order_url( $payment_id ), $order_number ) ) ) );
		} else {
			/* translators: 1. The URL for the order 2. The order number 3. The failure message returned from the license key generator */
			die( json_encode( array( 'error' => sprintf( __( 'Order <a href="%1$s" target="_blank">%2$s</a> was NOT licensed because "%3$s".', 'templify_sl' ), self::get_order_url( $payment_id ), $order_number, $number_generated ) ) ) );
		}
	}
	...
	*/
	public static function generate_license_keys( $payment_id, $download_id, $payment = false ) {
		...
		if ( empty( $payment_id ) ) {
			return esc_html__( 'Empty `$payment_id`', 'templify_sl' );
		}
		...
		if ( empty( $downloads ) ) {
			return esc_html__( 'No payment downloads found', 'templify_sl' );
		}
