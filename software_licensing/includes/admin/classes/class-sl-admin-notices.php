...
class EDD_Software_Licensing_Admin_Notices {
	...
	public function future_requirements() {
		...
		<div class="notice notice-warning edd-notice">
			<h2><?php esc_html_e( 'System Upgrades Required for Software Licensing', 'templify_sl' ); ?></h2>
			<p>
				<?php echo wp_kses( __( 'Your site needs to upgrade to <strong>at least</strong> the following in order to be ready for Software Licensing 3.9:', 'templify_sl' ), array( 'strong' => array() ) ); ?>
			</p>
			<ul>
			...
			<p>
				<?php echo wp_kses( __( 'While these versions are the <em>minimum</em> we will be supporting, we encourage you to update to the most recent versions available.', 'templify_sl' ), array( 'em' => array() ) ); ?>
			</p>

			<p>
				<a href="<?php echo esc_url( $dismiss_notice_url ); ?>"><?php esc_html_e( 'Dismiss Notice', 'templify_sl' ); ?></a>
			</p>
		</div>
		<?php
	}
	...
	public function notices() {
		...
		switch ( strtolower( $_GET['edd-message'] ) ) {
			...
			case 'deactivate':
				$message = __( 'License deactivated successfully.', 'templify_sl' );
				break;

			case 'activate':
				$message = __( 'License activated successfully.', 'templify_sl' );
				break;

			case 'enable':
				$message = __( 'License enabled successfully.', 'templify_sl' );
				break;

			case 'disable':
				$message = __( 'License disabled successfully.', 'templify_sl' );
				break;

			case 'renew':
				$message = __( 'License renewed successfully.', 'templify_sl' );
				break;

			case 'renewal_notice':
				$message = __( 'Renewal notice sent successfully.', 'templify_sl' );
				break;

			case 'license-updated':
				$message = __( 'License updated successfully.', 'templify_sl' );
				break;

			case 'delete':
				$message = __( 'License deleted successfully.', 'templify_sl' );

				break;

			case 'delete-error':
				$message = __( 'Error deleting license.', 'templify_sl' );

				break;

			case 'send-notice':
				$message = __( 'License renewal notice sent successfully.', 'templify_sl' );

				break;

			case 'set-lifetime':
				$message = __( 'License expiration set to lifetime successfully.', 'templify_sl' );
				break;

			case 'license-subscribed':
				$message = __( 'License successfully subscribed to email notices.', 'templify_sl' );
				break;

			case 'license-unsubscribed':
				$message = __( 'License successfully unsubscribed from email notices.', 'templify_sl' );
				break;
		}
