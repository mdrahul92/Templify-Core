...
function edd_sl_licenses_list() {
	...
	<div class="wrap">
		...
		<h1><?php esc_html_e( 'Easy Digital Download Licenses', 'templify_sl' ); ?></h1>
...
function edd_sl_render_license_view( $view, $callbacks ) {
	...
	if( ! current_user_can( 'view_licenses' ) ) {
		edd_set_error( 'edd-no-access', __( 'You are not permitted to view this data.', 'templify_sl' ) );
		$render = false;
	}
	...
	if ( isset( $_GET['license'] ) ) {
		...
		if ( empty( $new_license_id ) ) {
			edd_set_error( 'edd-invalid-license', __( 'Invalid license ID provided.', 'templify_sl' ) );
			$render = false;
		}
	}
	...
	?>
	<div class="wrap">
		<h2><?php _e( 'Manage License Details', 'templify_sl' ); ?></h2>
					...
					<ul id="edd-item-tab-wrapper-list" class="license-tab-wrapper-list">
						<?php foreach ( $license_tabs as $key => $tab ) : ?>
							...
							<li class="<?php echo sanitize_html_class( $class ); ?>">
								...
								$tab_title = sprintf( _x( 'License %s', 'License Details page tab title', 'templify_sl' ), esc_attr( $tab[ 'title' ] ) );
...
function edd_sl_licenses_view( $license ) {
	...
	<div class="info-wrapper item-section">
		<form id="edit-item-info" method="post" action="<?php echo admin_url( 'edit.php?post_type=download&page=edd-licenses&view=overview&license_id=' . $license->ID ); ?>">
			<div class="item-info">
				<table class="widefat striped">
					<tbody>
						<tr>
							<td class="row-title">
								<label for="tablecell"><?php _e( 'License Key:', 'templify_sl' ); ?></label>
							</td>
							<td style="word-wrap: break-word">
								...
								<span>
									...
									<?php $tool_tip_title   = __( 'Regenerate License Key', 'templify_sl'); ?>
									<?php $tool_tip_message = __( 'In the event that a user needs to have their license key changed, using this button will assign a new key to this license', 'templify_sl' ); ?>
						...
						<tr>
							<td class="row-title">
								<label for="tablecell"><?php _e( 'Purchase Date:', 'templify_sl' ); ?></label>
							</td>
							<td>
								<?php
								if ( false === $initial_payment ) {
									?>
									&mdash;
									<span alt="f223" class="edd-help-tip dashicons dashicons-editor-help" title="<strong><?php esc_html_e( 'Missing Purchase Date', 'templify_sl' ); ?></strong>:<br /><?php esc_html_e( 'The originating order for this license is missing or invalid.', 'templify_sl' ) ?>">
									<?php
								} else {
						...
						<tr>
							<td class="row-title">
								<label for="tablecell"><?php _e( 'Expiration Date:', 'templify_sl' ); ?></label>
							</td>
							<td>
								<?php
								...
								<span>&nbsp;&ndash;&nbsp;</span>
								<a href="#" class="edd-sl-edit-license-exp-date"><?php _e( 'Edit', 'templify_sl' ); ?></a>
								<?php endif; ?>

								<?php
								if ( $license->parent > 0 ) {
									printf( __( '(Set by <a href="%s">parent license</a>)', 'templify_sl' ), add_query_arg( 'license_id', $parent_license->ID ) );
								}
								?>

								<?php
								if( empty( $license->parent ) && ! $license->is_lifetime ) {
									echo sprintf( 'or&nbsp;<a href="%s&action=%s">' . __( 'Mark as Lifetime', 'templify_sl' ) . '</a>', $base, 'set-lifetime' );
								}
								?>
							</td>
						</tr>
						<tr>
							<td class="row-title">
								<label for="tablecell"><?php _e( 'License Term', 'templify_sl' ); ?>:</label>
							</td>
							<td>
						...
						<tr>
							<td class="row-title">
								<label for="tablecell"><?php _e( 'Product:', 'templify_sl' ); ?></label>
							</td>
							<td>
								<?php
						...
						<tr>
							<td class="row-title">
								<label for="tablecell"><?php _e( 'Activation Limit:', 'templify_sl' ); ?></label>
							</td>
							<td>
								<?php
						...
						<tr>
							<td class="row-title">
								<label for="tablecell"><?php _e( 'Status:', 'templify_sl' ); ?></label>
							</td>
							<td>
								<?php
								...
								if ( 'disabled' === $license->status ) {
									echo ' <em>(' . __( 'disabled', 'templify_sl' ) . ')</em>';
								}
								?>
								<span alt="f223" class="edd-help-tip dashicons dashicons-editor-help" title="<strong><?php _e( 'What does this mean', 'templify_sl'); ?></strong>:<br /><strong><?php _e( 'Active:', 'templify_sl' ); ?></strong> <?php _e( 'Indicates that a user has activated a site with this license.', 'templify_sl' ); ?><br /><strong> <?php _e( 'Inactive:', 'templify_sl' ); ?></strong> <?php _e( 'Indicates that this license is not currently activated on any sites.', 'templify_sl' ); ?></span><br /><strong><?php _e( 'Expired:', 'templify_sl' ); ?></strong> <?php _e( 'Indicates that this license has expired and can not be used unless renewed.', 'templify_sl' ); ?><br /><strong><?php _e( 'Disabled:', 'templify_sl' ); ?></strong> <?php _e( 'Indicates that this license has been administratively disabled.', 'templify_sl' ); ?>">
							</td>
						</tr>
						<tr>
							<td class="row-title">
								<label for="tablecell"><?php _e( 'Customer:', 'templify_sl' ); ?></label>
							</td>
							<td>
								<?php
						...
						<?php if( ! $license->is_lifetime ) : ?>
							<tr>
								<td class="row-title">
									<label for="tablecell"><?php _e( 'Email Notifications:', 'templify_sl' ); ?></label>
								</td>
								<td>
									<?php
									if( $unsubscribed ) {
										printf( __( 'Unsubscribed on %s', 'templify_sl' ), date_i18n( 'Y-n-d H:i:s', $unsubscribed ) );
									} else {
										_e( 'Subscribed', 'templify_sl' );
									}
									?>
									<span alt="f223" class="edd-help-tip dashicons dashicons-editor-help" title="<strong><?php _e( 'This indicates whether this customer will receive the license renewal email notifications configured in the Software Licensing settings tab.', 'templify_sl' ); ?>"></span>
									<?php
									$button_text   = $unsubscribed ? __( 'Subscribe', 'templify_sl' ) : __( 'Unsubscribe', 'templify_sl' );
						...
						<tr>
							<td class="row-title">
								<label for="tablecell"><?php _e( 'Actions:', 'templify_sl' ); ?></label>
							</td>
							<td class="edd-sl-license-card-actions">
								...
								if( ! $license->parent ) {
									if ( ! $license->is_lifetime ) {
										...
										if ( ! $unsubscribed && edd_sl_renewals_allowed() ) {
											$actions['renewal_notice'] = '<a href="#" id="edd_sl_send_renewal_notice" title="' . esc_attr__( 'Send a renewal notice for this license key', 'templify_sl' ) . '">' . esc_html__( 'Send Renewal Notice', 'templify_sl' ) . '</a>';
										}

										if ( 'disabled' !== $license->status ) {
											if( 'expired' !== $status ) {
												$actions['renew'] = sprintf( '<a href="%s&action=%s" title="' . __( 'Extend this license key\'s expiration date', 'templify_sl' ) . '">' . __( 'Extend', 'templify_sl' ) . '</a>', $base, 'renew', $license->ID );
											} else {
												$actions['renew'] = sprintf( '<a href="%s&action=%s">' . __( 'Renew', 'templify_sl' ) . '</a>', $base, 'renew' );
											}
										}
									...
									if( 'disabled' === $license->status ) {
										$actions['enable'] = sprintf( '<a href="%s&action=%s">' . __( 'Enable', 'templify_sl' ) . '</a>', $base, 'enable' );
									} else {
										$actions['disable'] = sprintf( '<a href="%s&action=%s">' . __( 'Disable', 'templify_sl' ) . '</a>', $base, 'disable' );
									}
								...
								if ( ! empty( $actions ) ) {
									...
									_e( 'No actions available for this license', 'templify_sl' );
								}
								?>
							</td>
						</tr>
						<tr class="edd-sl-license-card-notices">
							<td class="row-title">
								<label for="tablecell"><?php _e( 'Select Notice:', 'templify_sl' ); ?></label>
							</td>
							<td>
								...
								<input type="submit" class="button-secondary button" value="<?php _e( 'Send Notice', 'templify_sl' ); ?>" data-license-id="<?php echo esc_attr( $license->ID ); ?>" />
			...
			<div id="item-edit-actions" class="edit-item" style="float: right; margin: 10px 0 0; display: block;">
				...
				<input type="submit" name="edd_sl_update_license" id="edd_sl_update_license" class="button button-primary" value="<?php _e( 'Update License', 'templify_sl' ); ?>" />
	...
	<div id="edd-item-tables-wrapper" class="item-section">
		...
		<?php if ( $parent_has_license = (bool) edd_software_licensing()->get_license_key( $license->parent )  || ! empty( $has_children ) ) : ?>
			<h3>
				<?php if ( ! empty( $has_children ) ) : ?>
					<?php esc_html_e( 'Child Licenses:', 'templify_sl' ); ?>
				<?php else : ?>
					<?php esc_html_e( 'Parent License:', 'templify_sl' ); ?>
				<?php endif; ?>
				<span alt="f223" class="edd-help-tip dashicons dashicons-editor-help" title="<?php _e( 'This table shows licenses related to this license, including relevant bundled licenses.', 'templify_sl' ); ?>"></span>
			</h3>
			<table class="wp-list-table widefat striped related-licenses">
				<thead>
					<tr>
						<th><?php _e( 'Product', 'templify_sl' ); ?></th>
						<th><?php _e( 'License Key', 'templify_sl' ); ?></th>
					</tr>
				</thead>
				<tbody>
		...
		<h3>
			<?php _e( 'Licensed URLs:', 'templify_sl' ); ?>
			<span alt="f223" class="edd-help-tip dashicons dashicons-editor-help" title="<?php _e( 'Use this form to add a new site URL for this license. Once added, the site will be considered active and will be included in the total site count.', 'templify_sl' ); ?>"></span>
		</h3>
		<table class="wp-list-table widefat striped licensed-urls">
			<thead>
				<tr>
					<th><?php _e( 'Site URL', 'templify_sl' ); ?></th>
					<th><?php _e( 'Actions', 'templify_sl' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php
				...
				if( ! empty( $sites ) ) :
					...
					foreach( $sites as $site ) : ?>
						...
						<tr class="row<?php if( $i % 2 == 0 ) { echo ' alternate'; } ?>">
							...
							<td><a href="<?php echo wp_nonce_url( add_query_arg( array( 'edd_action' => 'deactivate_site', 'site_id' => $site->site_id, 'license' => $license->ID ) ), 'edd_deactivate_site_nonce' ); ?>"><?php _e( 'Deactivate Site', 'templify_sl' ); ?></a></td>
						</tr>
						<?php
						$i++;
					endforeach;
				else : ?>
					<tr class="row"><td colspan="2"><?php esc_html_e( 'This license has not been activated on any sites.', 'templify_sl' ); ?></td></tr>
				<?php endif; ?>
				<tr class="edd-sl-add-licensed-url-row">
					<td colspan="2" class="edd-sl-add-licensed-url-td">
						<form method="post">
							<input type="text" name="site_url" placeholder="<?php esc_html_e( 'New site URL (including https://)', 'templify_sl' ); ?>"/>
							...
							<input type="submit" class="button-secondary button" value="<?php _e( 'Add Site', 'templify_sl' ); ?>"/>
						</form>
					</td>
				</tr>
			</tbody>
		...
		<h3>
			<?php _e( 'Related Payments:', 'templify_sl' ); ?>
			<span alt="f223" class="edd-help-tip dashicons dashicons-editor-help" title="<?php _e( 'This table shows the most recent payments relevant to this license, including upgrades and renewals.', 'templify_sl' ); ?>"></span>
		</h3>
		<table class="wp-list-table widefat striped related-payments">
			...
			<thead>
				<tr>
					<th><?php _e( 'ID', 'templify_sl' ); ?></th>
					<th><?php _e( 'Amount', 'templify_sl' ); ?></th>
					<th><?php _e( 'Date', 'templify_sl' ); ?></th>
					<th><?php _e( 'Status', 'templify_sl' ); ?></th>
					<th><?php _e( 'Actions', 'templify_sl' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php if ( ! empty( $payments ) ) : ?>
						...
						<tr>
							...
							<td>
								<a href="<?php echo admin_url( 'edit.php?post_type=download&page=edd-payment-history&view=view-order-details&id=' . $payment->ID ); ?>">
									<?php _e( 'View Details', 'templify_sl' ); ?>
								</a>
					...
					<?php endforeach; ?>
				<?php else: ?>
					<tr><td colspan="5"><?php _e( 'No Payments Found', 'templify_sl' ); ?></td></tr>
				<?php endif; ?>
			</tbody>
		</table>
		...
		<h3>
			<?php _e( 'Upgrade Paths:', 'templify_sl' ); ?>
			<span alt="f223" class="edd-help-tip dashicons dashicons-editor-help" title="<?php printf( __( 'This table shows the available upgrade paths for this license, along with a link to each specific upgrade. Upgrade paths can be added through the %s edit screen.', 'templify_sl' ), strtolower( edd_get_label_singular() ) ); ?>"></span>
		</h3>
		<table class="wp-list-table widefat striped upgrades">
			<thead>
				<tr>
					<th><?php _e( 'Product', 'templify_sl' ); ?></th>
					<th><?php _e( 'Amount', 'templify_sl' ); ?></th>
					<th><?php _e( 'Upgrade Link', 'templify_sl' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php
				if ( $status == 'expired' ) {
					echo '<tr>';
					echo '<td colspan="3">' . __( 'This license must be renewed before it can be upgraded', 'templify_sl' ) . '</td>';
					echo '</tr>';
				} else {
					...
					if ( $upgrades && ! $license->parent ) {
						...
						echo '<tr>';
						echo '<td colspan="3">&nbsp;&mdash;&nbsp;' . __( 'Bundled licenses can not be upgraded individually', 'templify_sl' ) . '</td>';
						echo '</tr>';
					} else {
						echo '<tr>';
						echo '<td colspan="3">' . __( 'No upgrade path available', 'templify_sl' ) . '</td>';
						echo '</tr>';
					}
				}
				?>
		...
		<?php if( edd_sl_renewals_allowed() && ! $license->is_lifetime ) : ?>
			<h3>
				<label for="edd-sl-renewal-url"><?php esc_html_e( 'Renewal URL:', 'templify_sl' ); ?></label>
				<span alt="f223" class="edd-help-tip dashicons dashicons-editor-help" title="<?php printf( __( 'This URL can be provided to a customer for a direct link to your checkout screen with the license renewal pre-populated.', 'templify_sl' ), strtolower( edd_get_label_singular() ) ); ?>"></span>
			</h3>
		...
		<?php if( ! $unsubscribed && ! $license->is_lifetime ) : ?>
			<h3>
				<label for="edd-sl-unsubscribe-url"><?php esc_html_e( 'Unsubscribe URL:', 'templify_sl' ); ?></label>
				<span alt="f223" class="edd-help-tip dashicons dashicons-editor-help" title="<?php _e( 'This URL can be provided to a customer in order to unsubscribe from license renewal notification emails.', 'templify_sl' ); ?>"></span>
			</h3>
			<table class="wp-list-table widefat striped">
				<tbody>
					<tr>
...
function edd_sl_licenses_logs_view( $license ) {
	...
	<div class="license-logs-header">
		<span><?php printf( __( 'License Key: %s', 'templify_sl' ), $license->key ); ?></span>
	</div>
	...
	<div class="edd-item-info license-logs">
		<h3>
			<?php _e( 'Logged Entries:', 'templify_sl' ); ?>
			<span alt="f223" class="edd-help-tip dashicons dashicons-editor-help" title="<?php _e( 'This table shows log entries relevant to this license.', 'templify_sl' ); ?>"></span>
		</h3>
		<table class="wp-list-table widefat striped license-logs">
			<thead>
				<tr>
					<th><?php _e( 'ID', 'templify_sl' ); ?></th>
					<th><?php _e( 'Date/Time', 'templify_sl' ); ?></th>
					<th><?php _e( 'Entry', 'templify_sl' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php
				...
				if( ! empty( $logs ) ) {
					...
					echo '<tr><td colspan="3">' . __( 'This license has no log entries', 'templify_sl' ) . '</td></tr>';
				}
				?>
			</tbody>
		</table>
		...
		<div class="edd-item-info delete-license">
			<span class="delete-license-options">
				<p>
					...
					<label for="edd-sl-license-delete-confirm"><?php _e( 'Are you sure you want to delete this license?', 'templify_sl' ); ?></label>
				</p>
			...
			<span id="license-edit-actions">
				...
				<input type="submit" disabled="disabled" id="edd-sl-delete-license" class="button-primary" value="<?php _e( 'Delete License', 'templify_sl' ); ?>" />
				<a id="edd-sl-delete-license-cancel" href="<?php echo admin_url( 'edit.php?post_type=download&page=edd-licenses&view=overview&license_id=' . $license->ID ); ?>" class="delete"><?php _e( 'Cancel', 'templify_sl' ); ?></a>
			</span>
		</div>
	</form>
