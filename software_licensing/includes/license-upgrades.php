...
function edd_sl_add_upgrade_to_cart( $data ) {
	...
	if ( true === $allow_upgrade ) {
		...
		switch ( $allow_upgrade ) {
			case 'expired':
				edd_set_error( 'edd-sl-expired-license', __( 'Your license key is expired. It must be renewed before it can be upgraded.', 'templify_sl' ) );
				break;

			case 'invalid_price_id':
				edd_set_error( 'edd-sl-invalid-price-id', __( 'Invalid price ID specified for upgrade.', 'templify_sl' ) );
				break;

			case 'renewal':
				edd_set_error( 'edd-sl-unique-action', __( 'License renewals and upgrades must be purchased separately. Please complete your license renewal before upgrading it.', 'templify_sl' ) );
				break;

			default:
				edd_set_error( 'edd-sl-upgrade-error', __( 'Your upgrade could not be added to the cart.', 'templify_sl' ) );
				break;
		}
	}
...
function edd_sl_validate_upgrade_in_cart( $valid_data, $posted ) {
	...
	foreach ( $cart_items as $item ) {
		...
		if ( (int) $user_id !== (int) $license->user_id || ! is_user_logged_in() ) {
			...
			edd_set_error( 'edd_sl_invalid_user_id', __( 'Please log into your account to upgrade your license', 'templify_sl' ) );
			break;
...
function edd_sl_upgrade_details_cart_item( $item ) {
	...
	?>
		<div class="edd-sl-upgrade-details edd-sl-upgrade-details-cart">
				<span class="edd-sl-upgrade-label"><?php esc_html_e( 'Upgrading', 'templify_sl' ); ?>:</span>
...
function edd_sl_license_upgrade_cart_item_price_label( $label, $download_id, $options ) {
	...
	return $label . ' - ' . __( '<em>license upgrade</em>', 'templify_sl' );
}
...
function edd_sl_process_license_upgrade( $download_id = 0, $payment_id = 0, $type = 'default', $cart_item = array(), $cart_index = 0 ) {
			...
			foreach ( $child_licenses as $child_license ) {
				...
				if ( false !== $price_id_pos ) {
					...
					$child_license->add_log(
						__( 'License Upgraded', 'templify_sl' ),
						/* translators: the new price ID; payment ID*/
						sprintf( __( 'Price ID updated to %1$d via Payment ID %2$s', 'templify_sl' ), $child_license->price_id, $payment_id )
					);
				}
			}
		}
	...
	$note = sprintf( __( 'License upgraded from %s to %s', 'templify_sl' ), $old_product, $new_product );
...
?>
	<div class="edd-admin-box-inside">
		<p>
			<?php printf( '<strong>%s:</strong> <a href="%s">#%s</a>', __( 'Upgraded from', 'templify_sl' ), $view_url, edd_get_payment_number( $upgraded_from ) ); ?>&nbsp;
		</p>
	</div>
<?php
	endif;
...
?>
	<div class="edd-admin-box-inside">
		<p>
			<?php printf( '<strong>%s:</strong> <a href="%s">#%s</a>', __( 'Upgraded to', 'templify_sl' ), $view_url, edd_get_payment_number( $upgraded_to ) ); ?>&nbsp;
		</p>
	</div>
<?php
	endif;
...
function edd_sl_add_upgrade_link( $download_id, $args ) {
	...
	if( count( $licenses ) == 1 ) {
		echo '<span class="edd-sl-upgrade-link"><a href="' . esc_url( edd_sl_get_license_upgrade_list_url( $licenses[0]->ID ) ) . '">' . __( 'Upgrade your existing license', 'templify_sl' ) . '</a></span>';
	} else {
		foreach( $licenses as $license ) {
			...
			echo '<span class="edd-sl-upgrade-link"><a href="' . esc_url( edd_sl_get_license_upgrade_list_url( $license->ID )  ) . '">' . sprintf( __( 'Upgrade license ending in %s', 'templify_sl' ), $license_key ) . '</a></span>';
		}
	}
}
