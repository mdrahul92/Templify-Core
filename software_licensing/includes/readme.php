...
function edd_sl_render_readme_cache_status() {
	...
	if ( ! empty( $readme ) ) {
		$message = sprintf( __( 'the file has been cached. %1$sClear cached file%2$s', 'templify_sl' ), '<button class="button button-secondary">', '</button>' );
	} else {
		$message = __( 'the file is not cached.', 'templify_sl' );
	}

	echo '<div class="alignright" id="edd_readme_cache">';
	printf( wpautop( '<strong>%s</strong> %s' ), __( 'Cache:', 'templify_sl' ), $message );
	echo '</div>';
}
...
function edd_sl_delete_readme_transient() {
	...
	if ( empty( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'edd_sl_readme_cache_nonce' ) || ! current_user_can( 'edit_products' ) ) {
		wp_send_json_error(
			array(
				'message' => __( 'Something went wrong. You may not have permission to perform this action.', 'templify_sl' ),
			),
			403
		);
	}
	...
	if ( $response ) {
		wp_send_json_success(
			array(
				'message' => __( 'The cache has been deleted.', 'templify_sl' ),
			)
		);
	}
	wp_send_json_error(
		array(
			'message' => __( 'There was an error when deleting the cache. It may have already been deleted.', 'templify_sl' ),
		)
	);
}
