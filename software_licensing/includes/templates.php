...
function edd_sl_add_key_column() {
	echo '<th class="edd_license_key">' . __( 'License Keys', 'templify_sl' ) . '</th>';
}
...
function edd_sl_site_management_links( $order_or_order_id, $purchase_data = array() ) {
	...
	if ( $downloads ) :
		...
		if ( edd_is_payment_complete( $order_id ) && $licensing->get_licenses_of_purchase( $order_id ) ) {
			echo '<a href="' . esc_url( $manage_licenses_url ) . '">' . esc_html__( 'View Licenses', 'templify_sl' ) . '</a>';
		} else {
			echo '&ndash;';
		}
		echo '</td>';
...
function edd_sl_blocks_management_links( $order ) {
	...
	<div class="edd-software-licensing__order">
		<a href="<?php echo esc_url( $manage_licenses_url ); ?>"><?php esc_html_e( 'View Licenses', 'templify_sl' ); ?></a>
	</div>
	<?php
}
...
function edd_sl_show_keys_on_receipt( $order_or_payment, $edd_receipt_args ) {
	...
	if( ! empty( $licenses ) ) {
		echo '<tr class="edd_license_keys">';
			echo '<td colspan="2"><strong>' . __( 'License Keys:', 'templify_sl' ) . '</strong></td>';
		echo '</tr>';
			...
			echo '<tr class="edd_license_key">';
				echo '<td>';
					...
					if( 'expired' == $license->status ) {
						echo '<span class="edd_sl_license_key_expired">&nbsp;(' . __( 'expired', 'templify_sl' ) . ')</span>';
					} elseif( 'disabled' === $license->status ) {
						echo '<span class="edd_sl_license_key_revoked">&nbsp;(' . __( 'disabled', 'templify_sl' ) . ')</span>';
					}
				echo '</td>';
				if( $license ) {
					echo '<td>';
					...
					echo '<td><span class="edd_sl_license_key edd_sl_none">' . __( 'none', 'templify_sl' ) . '</span></td>';
				}
			echo '</tr>';
		}
	}
...
function edd_sl_show_keys_on_block_receipt( $order ) {
	...
	<div class="edd-blocks__row edd-blocks-receipt__row-item">
		<div class="edd-blocks__row-label"><?php esc_html_e( 'License Keys:', 'templify_sl' ); ?></div>
	</div>
	<?php
	foreach ( $licenses as $license ) {
		?>
		<div class="edd-blocks__row edd-blocks-receipt__row-item">
			<div class="edd-blocks__row-sublabel">
				...
				if ( 'expired' === $license->status ) {
					echo '<span class="edd_sl_license_key_expired">&nbsp;(' . esc_html__( 'expired', 'templify_sl' ) . ')</span>';
				} elseif ( 'disabled' === $license->status ) {
					echo '<span class="edd_sl_license_key_revoked">&nbsp;(' . esc_html__( 'disabled', 'templify_sl' ) . ')</span>';
				}
				?>
			</div>
