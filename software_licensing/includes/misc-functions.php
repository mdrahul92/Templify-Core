	...
	switch( $meta_key ) {
		...
		case '_edd_sl_site_count':
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG && ! $edd_is_checkout ) {
				...
				trigger_error( __( 'The _edd_sl_site_count postmeta is <strong>deprecated</strong> since EDD Software Licensing 2.4.8! Use edd_software_licensing->get_site_count( $license_id ) instead.', 'templify_sl' ) );
		...
		if ( empty( $initial_notification ) ) {
			$notifications->maybe_add_local_notification(
				array(
					...
					'conditions' => '',
					'type'       => 'info',
					'title'      => __( 'Database Optimization in Progress', 'templify_sl' ),
					'content'    => __( 'Software Licensing is currently optimizing your license meta table in the background. This process may take a while to complete depending on the number of licenses on your site. We\'ll let you know when the process is complete.', 'templify_sl' ),
				)
			);
		}
	} else {
	...
	if ( false !== $notifications ) {
		...
		$notifications->maybe_add_local_notification(
			array(
				...
				'conditions' => '',
				'type'       => 'success',
				'title'      => __( 'Database Optimization Complete!', 'templify_sl' ),
				'content'    => __( 'Software Licensing has completed the database optimization for the license meta table.', 'templify_sl' ),
			)
		);
	} else {
...
function _edd_sl_legacy_ids_cleanup_admin_notices() {
	...
	if ( get_option( 'edd_sl_legacy_id_cleanup_running' ) ) {
		...
		<div class="notice notice-info edd-notice">
			<h2><?php esc_html_e( 'Easy Digital Downloads Database Optimization in Progress', 'templify_sl' ); ?></h2>
			<p>
				<?php esc_html_e( 'Software Licensing is currently optimizing your license meta table in the background. This process may take a while to complete depending on the number of licenses on your site. We\'ll let you know when the process is complete.', 'templify_sl' ); ?>
			</p>
		</div>
		<?php
		...
		?>
		<div class="notice notice-info edd-notice">
			<h2><?php esc_html_e( 'Database Optimization Complete!', 'templify_sl' ); ?></h2>
			<p>
				<?php esc_html_e( 'Software Licensing has completed the database optimization for the license meta table.', 'templify_sl' ); ?> <a class="button button-secondary" href="<?php echo esc_url( $dismiss_notice_url ); ?>"><?php esc_html_e( 'Dismiss Notice', 'templify_sl' ); ?></a>
			</p>
		</div>
		<?php
	}
