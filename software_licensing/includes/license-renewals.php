...
function edd_sl_get_renewal_notice_periods() {
	$periods = array(
		'+1day'    => __( 'One day before expiration', 'templify_sl' ),
		'+2days'   => __( 'Two days before expiration', 'templify_sl' ),
		'+3days'   => __( 'Three days before expiration', 'templify_sl' ),
		'+1week'   => __( 'One week before expiration', 'templify_sl' ),
		'+2weeks'  => __( 'Two weeks before expiration', 'templify_sl' ),
		'+1month'  => __( 'One month before expiration', 'templify_sl' ),
		'+2months' => __( 'Two months before expiration', 'templify_sl' ),
		'+3months' => __( 'Three months before expiration', 'templify_sl' ),
		'expired'  => __( 'At the time of expiration', 'templify_sl' ),
		'-1day'    => __( 'One day after expiration', 'templify_sl' ),
		'-2days'   => __( 'Two days after expiration', 'templify_sl' ),
		'-3days'   => __( 'Three days after expiration', 'templify_sl' ),
		'-1week'   => __( 'One week after expiration', 'templify_sl' ),
		'-2weeks'  => __( 'Two weeks after expiration', 'templify_sl' ),
		'-1month'  => __( 'One month after expiration', 'templify_sl' ),
		'-2months' => __( 'Two months after expiration', 'templify_sl' ),
		'-3months' => __( 'Three months after expiration', 'templify_sl' ),
	);
...
function edd_sl_get_renewal_notice( $notice_id = 0 ) {
	...
	$defaults = array(
		'subject'     => __( 'Your License Key is About to Expire', 'templify_sl' ),
		'send_period' => '+1month',
		'message'     => 'Hello {name},
...
function edd_sl_match_renewal_email( $valid_data, $posted ) {
	...
	foreach ( $keys as $key ) {
		...
		if ( ! in_array( $posted['edd_email'], $emails ) ) {
			edd_set_error( 'email_match', sprintf( __( 'The specified email is not authorized to renew license %s.', 'templify_sl' ), $key ) );
		}
	}
}
...
function edd_sl_add_renewal_to_cart( $license_id = 0, $by_key = false ) {
	...
	if ( ! edd_sl_renewals_allowed() ) {
		return new WP_Error( 'renewals_disabled', __( 'Renewals are not allowed on this site.', 'templify_sl' ) );
	}

	$license = edd_software_licensing()->get_license( $license_id, $by_key );

	if ( false === $license ) {
		return new WP_Error( 'missing_license', __( 'No license ID supplied or invalid key provided', 'templify_sl' ) );
	}

	if ( 'disabled' === $license->status ) {
		return new WP_Error( 'license_disabled', __( 'The supplied license has been disabled and cannot be renewed', 'templify_sl' ) );
	}
	...
	if ( ! in_array( $payment->status, array( 'publish', 'complete', 'partially_refunded' ), true ) ) {
		return new WP_Error( 'payment_not_complete', __( 'The purchase record for this license is not marked as complete', 'templify_sl' ) );
	}

	if ( 'publish' !== $license->get_download()->post_status ) {
		return new WP_Error( 'license_disabled', __( 'The download for this license is not published', 'templify_sl' ) );
	}
	...
	if ( empty( $license->download_id ) ) {
		return new WP_Error( 'no_download_id', __( 'There does not appear to be a download ID attached to this license key', 'templify_sl' ) );
	}
	...
	if ( ! edd_item_in_cart( $license->download_id, $options ) ) {
		return new WP_Error( 'not_in_cart', __( 'The download for this license is not in the cart or could not be added', 'templify_sl' ) );
	}
	...
	return new WP_Error( 'renewal_error', __( 'Something went wrong while attempting to apply the renewal', 'templify_sl' ) );
}
...
function edd_sl_renewal_details_cart_item( $item ) {
	...
	?>
		<div class="edd-sl-renewal-details edd-sl-renewal-details-cart">
				<span class="edd-sl-renewal-label"><?php esc_html_e( 'Renewing', 'templify_sl' ); ?>:</span>
...
function edd_sl_cart_items_renewal_row() {
	...
	$message = sprintf( __( 'License renewal discount: %s', 'templify_sl' ), $formatted_discount_amount );
...
function edd_sl_payment_details_inner( $payment_id = 0 ) {
	...
	<div class="edd-admin-box-inside">
		<p>
			<strong><?php _e( 'Was renewal:', 'templify_sl' ); ?></strong>&nbsp;
			<span><?php echo $was_renewal ? __( 'Yes', 'templify_sl' ) : __( 'No', 'templify_sl' ); ?></span>
		</p>
	</div>
	<?php
}
...
function edd_sl_dynamic_email_strings() {
	$strings  = array(
		'{name}'            => __( 'The customer\'s first name', 'templify_sl' ),
		'{fullname}'        => __( 'The customer\'s full name', 'templify_sl' ),
		'{license_key}'     => __( 'The license key that needs renewed', 'templify_sl' ),
		'{product_name}'    => __( 'The name of the product the license key belongs to', 'templify_sl' ),
		'{expiration}'      => __( 'The expiration date for the license key', 'templify_sl' ),
		'{renewal_link}'    => __( 'URL to the renewal checkout page', 'templify_sl' ),
		'{renewal_url}'     => __( 'Raw URL of the renewal checkout page', 'templify_sl' ),
		'{unsubscribe_url}' => __( 'Raw URL to unsubscribe from email notifications for the license', 'templify_sl' ),
	);
	$discount = edd_get_option( 'edd_sl_renewal_discount', false );
	if ( ! empty( $discount ) ) {
		$strings['{renewal_discount}'] = __( 'The renewal discount, including the `%` symbol.', 'templify_sl' );
	}
...
function edd_sl_process_renewal_email_unsubscribe() {
	...
	wp_die( __( 'You have been successfully unsubscribed from renewal notification emails for this license key.', 'templify_sl' ), __( 'Unsubscribed', 'templify_sl' ), 200 );
}
