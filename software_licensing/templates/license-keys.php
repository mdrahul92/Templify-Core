	...
	<table id="edd_sl_license_keys" class="edd_sl_table edd-table">
		<thead>
			<tr class="edd_sl_license_row">
				...
				<th class="edd_sl_item"><?php esc_html_e( 'Item', 'templify_sl' ); ?></th>
				<th class="edd_sl_details"><?php esc_html_e( 'License Details', 'templify_sl' ); ?></th>
				...
				<tr class="edd_sl_license_row">
					...
					<td>
						...
						<?php if ( ! empty( $child_licenses ) ) : ?>
							<strong><?php esc_html_e( 'Bundle Licenses', 'templify_sl' ); ?></strong>
...
>>>>>>> origin/master
											...
											if ( ! edd_software_licensing()->force_increase() ) {
												...
												?>
												&nbsp;&ndash;&nbsp;<a href="<?php echo esc_url( $url ); ?>"><?php esc_html_e( 'Manage Sites', 'templify_sl' ); ?></a>
												<?php
											}
											?>
										</span>
...
>>>>>>> origin/master
					<td>
						...
						<div class="edd_sl_item_expiration">
							<span class="edd_sl_expires_label edd_sl_expiries_label"><?php 'expired' === $license->status ? esc_html_e( 'Expired:', 'templify_sl' ) : esc_html_e( 'Expires:', 'templify_sl' ); ?>&nbsp;</span>
							<?php if ( $license->is_lifetime ) : ?>
								<?php _e( 'Never', 'templify_sl' ); ?>
							<?php else: ?>
								<?php echo esc_html( date_i18n( 'F j, Y', $license->expiration ) ); ?>
							<?php endif; ?>
						</div>
						<span class="edd_sl_limit_label"><?php esc_html_e( 'Activations:', 'templify_sl' ); ?>&nbsp;</span>
						<span class="edd_sl_limit_used"><?php echo esc_html( $license->activation_count ); ?></span>
						<span class="edd_sl_limit_sep">&nbsp;/&nbsp;</span>
						<span class="edd_sl_limit_max"><?php echo esc_html( 0 !== $license->activation_limit ? $license->activation_limit : __( 'Unlimited', 'templify_sl' ) ); ?></span>
						<?php if ( ! edd_software_licensing()->force_increase() && ( ! in_array( $license->status, array( 'expired', 'disabled' ), true ) ) ) : ?>
							<br/><a href="<?php echo esc_url( add_query_arg( array( 'license_id' => $license->ID, 'action' => 'manage_licenses', 'payment_id' => $payment_id ), get_permalink( edd_get_option( 'purchase_history_page' ) ) ) ); ?>"><?php _e( 'Manage Sites', 'templify_sl' ); ?></a>
						<?php elseif ( 'expired' === $license->status ) : ?>
							<br/><span class="edd_sl_no_management"><?php esc_html_e( 'Renew to manage sites', 'templify_sl' ); ?></span>
						<?php else : ?>
							<br/><span class="edd_sl_no_management"><?php esc_html_e( 'Unable to manage sites', 'templify_sl' ); ?></span>
						<?php endif; ?>
						<?php if ( edd_sl_license_has_upgrades( $license->ID ) && 'expired' !== $license->status ) : ?>
							<span class="edd_sl_limit_sep">&nbsp;&ndash;&nbsp;</span>
							<a href="<?php echo esc_url( add_query_arg( array( 'view' => 'upgrades', 'license_id' => $license->ID, 'action' => 'manage_licenses', 'payment_id' => $payment_id ), get_permalink( edd_get_option( 'purchase_history_page' ) ) ) ); ?>"><?php _e( 'View Upgrades', 'templify_sl' ); ?></a>
						<?php elseif ( edd_sl_license_has_upgrades( $license->ID ) && 'expired' === $license->status ) : ?>
							<span class="edd_sl_limit_sep">&nbsp;&ndash;&nbsp;</span>
							<span class="edd_sl_no_upgrades"><?php esc_html_e( 'Renew to upgrade', 'templify_sl' ); ?></span>
						<?php endif; ?>
						<?php if ( $renewals_allowed ) : ?>
							<?php if ( 'expired' === $license->status && edd_software_licensing()->can_renew( $license->ID ) ) : ?>
								...
								<a href="<?php echo esc_url( $license->get_renewal_url() ); ?>"><?php esc_html_e( 'Renew license', 'templify_sl' ); ?></a>
							<?php elseif ( ! $license->is_lifetime && edd_software_licensing()->can_extend( $license->ID ) ) : ?>
								<span class="edd_sl_key_sep">&nbsp;&ndash;&nbsp;</span>
								<a href="<?php echo esc_url( $license->get_renewal_url() ); ?>"><?php esc_html_e( 'Extend license', 'templify_sl' ); ?></a>
							<?php endif; ?>
						<?php endif; ?>
						<br/>
						<a class="edd_sl_purchase_number" href="<?php echo esc_url( edd_get_success_page_uri( '?payment_key=' . edd_get_payment_key( $payment_id ) ) ); ?>" title="<?php esc_attr_e( 'View Purchase Record', 'templify_sl' ); ?>"><?php printf( __( 'Purchase #%s', 'templify_sl' ), edd_get_payment_number( $payment_id ) ); ?></a>
		...
		<?php else: ?>
			...
			<tr class="edd_sl_license_row">
				<td colspan="2"><?php esc_html_e( 'You currently have no licenses', 'templify_sl' ); ?></td>
			</tr>
		<?php endif; ?>
	</table>
...
<?php else : ?>
	<p class="edd-alert edd-alert-warn">
		<?php esc_html_e( 'You must be logged in to view license keys.', 'templify_sl' ); ?>
	</p>
