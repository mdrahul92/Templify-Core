...
?>
<p><a href="<?php echo esc_url( remove_query_arg( array( 'license_id', 'edd_sl_error', '_wpnonce' ) ) ); ?>" class="edd-manage-license-back edd-submit button <?php echo esc_attr( $color ); ?>"><?php _e( 'Go back', 'templify_sl' ); ?></a></p>
<?php edd_sl_show_errors(); ?>
<h5 class="edd-sl-manage-license-header"><?php _e( 'Manage License', 'templify_sl' ); ?></h5>
<p class="edd-sl-manage-license-details">
	<span class="edd-sl-manage-license-key"><?php _e( 'License', 'templify_sl' ); ?>: <?php echo '<code>' . $license->key . '</code>'; ?></span>
	<span class="edd-sl-manage-license-product"><?php _e( 'Product', 'templify_sl' ); ?>: <span><?php echo $download->get_name(); ?></span></span>
</p>
<table id="edd_sl_license_sites" class="edd_sl_table edd-table">
	<thead>
		<tr class="edd_sl_license_row">
			...
			<th class="edd_sl_url"><?php _e( 'Site URL', 'templify_sl' ); ?></th>
			<th class="edd_sl_actions"><?php _e( 'Actions', 'templify_sl' ); ?></th>
	...
	<?php if ( $sites ) : ?>
		<?php foreach ( $sites as $site ) : ?>
			<tr class="edd_sl_license_row">
				...
				<td><a href="<?php echo wp_nonce_url( add_query_arg( array( 'edd_action' => 'deactivate_site', 'site_id' => $site->site_id, 'license' => $license_id ) ), 'edd_deactivate_site_nonce', '_wpnonce' ); ?>"><?php _e( 'Deactivate Site', 'templify_sl' ); ?></a></td>
	...
	<?php else: ?>
		<tr class="edd_sl_license_row">
			<?php do_action( 'edd_sl_license_sites_row_start', $license_id ); ?>
			<td colspan="2"><?php _e( 'No sites have been activated for this license', 'templify_sl' ); ?></td>
			<?php do_action( 'edd_sl_license_sites_row_end', $license_id ); ?>
		</tr>
	<?php endif; ?>
</table>
...
<form method="post" id="edd_sl_license_add_site_form" class="edd_sl_form">
	<div>
		<span><?php _e( 'Use this form to authorize a new site URL for this license. Enter the full site URL.', 'templify_sl' ); ?></span>
		<input type="text" name="site_url" class="edd-input" value="https://"/>
		<input type="submit" class="button-primary button" value="<?php _e( 'Add Site', 'templify_sl' ); ?>"/>
