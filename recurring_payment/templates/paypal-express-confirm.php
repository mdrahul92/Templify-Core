...
<div class="edd-confirm-details" id="billing_info">
	<h3><?php _e( 'Please confirm your subscription', 'templify-recurring' ); ?></h3>
	<p><strong><?php echo $edd_checkout_details['FIRSTNAME'] ?> <?php echo $edd_checkout_details['LASTNAME'] ?></strong><br />
	<?php _e( 'PayPal Status:', 'templify-recurring' ); ?> <?php echo $edd_checkout_details['PAYERSTATUS'] ?><br />
	<?php _e( 'Email:', 'templify-recurring' ); ?> <?php echo $edd_checkout_details['EMAIL'] ?></p>
</div>
<table id="order_summary" class="edd-table">
	<thead>
		<tr>
			...
			<th><?php _e( 'Recurring Amount', 'templify-recurring' ); ?></th>
		</tr>
	</thead>
	<tbody>
		...
		<tr>
			...
			<td>
				...
				<?php if( $subscription->bill_times > 1 ) : ?>
					...
					<br/>
					<span class="edd_subscription_bill_times"><?php printf( _n( '%d Time', '%d Times', $subscription->bill_times, 'templify-recurring' ), $subscription->bill_times ); ?></span>
				<?php endif; ?>
			</td>
		</tr>
		<?php endforeach; ?>
...
<form action="<?php echo esc_url( add_query_arg( 'edd-confirm', 'paypal_express' ) ); ?>" method="post">
	...
	<input type="submit" value="<?php _e( 'Confirm', 'templify-recurring' ); ?>" />
</form>
