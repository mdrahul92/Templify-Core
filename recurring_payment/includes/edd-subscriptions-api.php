...
class EDD_Subscriptions_API extends EDD_API {
	...
	private function get_status_from_url() {
		...
		if ( null !== $input_status && '' === $status ) {
			$error['error'] = sprintf( __( '\'%s\' is not a valid status.', 'templify-recurring' ), $input_status );
	...
	public function add_edd_subscription_endpoint( $data, $query_mode, $api_object ) {
		...
		if( ! empty( $queried_c ) && ( ! $customer || ! $customer->id > 0 ) ) {
			...
			$error['error'] = sprintf( __( 'No customer found for %s!', 'templify-recurring' ), $queried_c );
		...
		if ( $subscriptions ) {
			...
			$error['error'] = sprintf( __( 'No subscriptions found for %s!', 'templify-recurring' ), $queried_c );

			return $error;

		} else {

			$error['error'] = __( 'No subscriptions found!', 'templify-recurring' );
