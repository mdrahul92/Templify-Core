		...
		jQuery(document).ready(function($) {
			$('select[name=edd-payment-status]').change(function() {
				...
				if( 'refunded' == $(this).val() ) {
					...
					cancel_sub_container.append( '<label for="edd_recurring_cancel_subscription"><?php echo esc_js( __( 'Cancel Subscription', 'templify-recurring' ) ); ?></label></div>' );
...
function edd_recurring_show_cancel_checkbox( $order ) {
	...
	foreach ( $subscriptions as $sub ) {
		$label = sprintf(
			...
			__( 'Cancel Subscription ID #%1$d (%2$s)', 'templify-recurring' ),
			$sub->id,
	...
	foreach ( $subs as $sub ) {
		...
		$note = edd_add_note(
			array(
				...
				'content'     => sprintf( __( 'Subscription %d cancelled because of refund.', 'templify-recurring' ), $sub->id ),
...
function edd_recurring_cancel_subscription_during_refund( $payment ) {
	...
	if ( $is_sub ) {
		...
		foreach ( $subs as $edd_sub ) {
			...
			/* translators: the subscription ID. */
			$payment->add_note( sprintf( __( 'Subscription %d cancelled because of refund.', 'templify-recurring' ), $edd_sub->id ) );
		...
		/* translators: the subscription ID. */
		$payment->add_note( sprintf( __( 'Subscription %d cancelled because of refund.', 'templify-recurring' ), $sub_id ) );
	}
}
