...
function edd_upgrade_render_recurring_update_order_item_status() {
	...
	if ( $migration_complete ) {
		...
		<div id="edd-recurring-migration-complete" class="notice notice-success">
			<p>
				...
				printf(
					'<strong>%s:</strong> %s',
					esc_html__( 'Migration complete', 'templify-recurring' ),
					esc_html__( 'You have already completed the subscription order items upgrade.', 'templify-recurring' )
				);
				?>
			</p>
		</div>
	...
	<div id="edd-recurring-migration-ready" class="notice notice-success" style="display: none;">
		<p>
			...
			printf(
				'<strong>%s:</strong> %s',
				esc_html__( 'Database Upgrade Complete', 'templify-recurring' ),
				esc_html__( 'The order items migration has been completed. You may now leave this page.', 'templify-recurring' )
			);
			?>
		</p>
	</div>
	...
	<div id="edd-migration-nav-warn" class="notice notice-info">
		<p>
			...
			printf(
				'<strong>%s:</strong> %s',
				esc_html__( 'Important', 'templify-recurring' ),
				esc_html__( 'Please leave this screen open and do not navigate away until the process completes.', 'templify-recurring' )
			);
			?>
		</p>
	</div>
	...
	<div class="metabox-holder">
		<div class="postbox">
			<h2 class="hndle"><?php esc_html_e( 'Update subscription records', 'templify-recurring' ); ?></h2>
			<div class="inside update-subscription-records-control">
				<p>
					<?php esc_html_e( 'This update will change the order items status in the database for renewal orders.', 'templify-recurring' ); ?>
				</p>
					...
					<span class="step-instructions-wrapper">
						...
						<?php if ( ! $migration_complete ) : ?>
							<span class="edd-migration allowed">
								<input type="submit" id="update-order-items-submit" value="<?php esc_html_e( 'Update Order Items', 'templify-recurring' ); ?>" class="button-primary"/>
							</span>
						<?php else : ?>
							<input type="submit" disabled id="migrate-logs-submit" value="<?php esc_html_e( 'Update Order Items', 'templify-recurring' ); ?>" class="button-secondary"/>
							&mdash; <?php esc_html_e( 'Order items have already been updated.', 'templify-recurring' ); ?>
						<?php endif; ?>
