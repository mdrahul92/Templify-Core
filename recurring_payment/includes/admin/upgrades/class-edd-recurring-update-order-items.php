...
class EDD_Recurring_Update_Order_Items extends EDD_Batch_Export {
	...
	public function process_step() {
		...
		if ( ! $this->can_export() ) {
			wp_die(
				esc_html__( 'You do not have permission to run this upgrade.', 'templify-recurring' ),
				esc_html__( 'Error', 'templify-recurring' ),
				array( 'response' => 403 )
			);
		}
		...
		if ( $had_data ) {
			...
			$this->done    = true;
			$this->message = __( 'Subscription records have been successfully updated.', 'templify-recurring' );
