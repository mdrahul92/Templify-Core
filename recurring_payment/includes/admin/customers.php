...
function edd_recurring_customer_subscriptions_list( $customer ) {
	...
	if ( ! $subscriptions ) {
		?>
		<p><?php esc_html_e( 'This customer has no subscriptions.', 'templify-recurring' ); ?></p>
		<?php
		return;
	}
	?>
	<h3><?php esc_html_e( 'Subscriptions', 'templify-recurring' ); ?></h3>
	<table class="wp-list-table widefat striped downloads">
		<thead>
			<tr>
				<th><?php esc_html_e( 'Subscription', 'templify-recurring' ); ?></th>
				<th><?php echo esc_html( edd_get_label_singular() ); ?></th>
				<th><?php esc_html_e( 'Billing Details', 'templify-recurring' ); ?></th>
				<th><?php esc_html_e( 'Order', 'templify-recurring' ); ?></th>
				<th><?php esc_html_e( 'Renewal Date', 'templify-recurring' ); ?></th>
			...
			foreach ( $subscriptions as $subscription ) :
				...
				<tr>
					<td>
						...
						<div class="row-actions"><span class="edit"><a href="<?php echo esc_url( $subscription_url ); ?>"><?php esc_html_e( 'View Details', 'templify-recurring' ); ?></div>
					</td>
...
function edd_recurring_register_subscription_customer_tab( $tabs ) {
	$tabs['subscriptions'] = array(
		...
		'title'    => __( 'Subscriptions', 'templify-recurring' ),
	);
...
function edd_recurring_customer_profile_ids( $customer ) {
	...
	?>
	<h3><?php esc_html_e( 'Recurring Profiles', 'templify-recurring' ); ?></h3>
	<table class="wp-list-table widefat striped downloads">
		<thead>
			<tr>
				<th><?php esc_html_e( 'Gateway', 'templify-recurring' ); ?></th>
				<th style="width: 150px;"><?php esc_html_e( 'Profile ID', 'templify-recurring' ); ?></th>
			</tr>
		</thead>
		<tbody>
...
function edd_recurring_anonymize_customer( $customer ) {
	...
	foreach ( $subscriptions as $subscription ) {
		if ( $subscription->can_cancel() ) {
			$subscription->cancel();
			$subscription->add_note( __( 'Subscription has been cancelled due to customer requesting anonymization.', 'templify-recurring' ) );
		}
	}
}
