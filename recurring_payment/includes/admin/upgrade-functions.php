...
function edd_show_recurring_upgrade_notices() {
	...
	if ( ! edd_has_upgrade_completed( 'upgrade_24_subscriptions' ) ) {
		...
		printf(
			'<div class="updated"><p>' . __( 'Easy Digital Downloads needs to upgrade the subscriptions database, click <a href="%s">here</a> to start the upgrade.', 'templify-recurring' ) . '</p></div>',
	...
	if ( ! edd_has_upgrade_completed( 'fix_24_stripe_customers' ) ) {
		...
		printf(
			'<div class="updated"><p>' . __( 'Easy Digital Downloads needs to upgrade the subscription customer database, click <a href="%s">here</a> to start the upgrade.', 'templify-recurring' ) . '</p></div>',
	...
	if ( ! edd_has_upgrade_completed( 'recurring_27_subscription_meta' ) ) {
		...
		printf(
			'<div class="updated"><p>' . __( 'Easy Digital Downloads needs to upgrade the subscription payments database, click <a href="%s">here</a> to start the upgrade.', 'templify-recurring' ) . '</p></div>',
	...
	if ( ! edd_has_upgrade_completed( 'recurring_paypalproexpress_logs' ) ) {
		...
		printf(
			'<div class="updated"><p>' . __( 'Easy Digital Downloads needs to upgrade the payment gateway error logs, click <a href="%s">here</a> to start the upgrade.', 'templify-recurring' ) . '</p></div>',
	...
	if ( ! edd_has_upgrade_completed( 'recurring_add_tax_columns_to_subs_table' ) ) {
		...
		printf(
			'<div class="updated"><p>' . __( 'Easy Digital Downloads needs to upgrade subscriptions table, click <a href="%s">here</a> to start the upgrade.', 'templify-recurring' ) . '</p></div>',
	...
	if ( ! edd_has_upgrade_completed( 'recurring_cancel_subs_if_times_met' ) ) {
		...
		printf(
			'<div class="updated"><p>' . __( 'Easy Digital Downloads wants to check to see if any subscriptions need to be set to complete. Click <a href="%s">here</a> to start.', 'templify-recurring' ) . '</p></div>',
	...
	if ( ! edd_has_upgrade_completed( 'recurring_update_price_id_column' ) ) {
		...
		printf(
			'<div class="updated"><p>' . __( 'Easy Digital Downloads needs to update the subscriptions table, click <a href="%s">here</a> to start the upgrade.', 'templify-recurring' ) . '</p></div>',
	...
	if ( function_exists( 'edd_get_orders' ) && ! edd_has_upgrade_completed( 'recurring_update_order_item_status' ) ) {
		...
		if ( $order_items ) {
			...
			printf(
				'<div class="updated"><p>' . __( 'Easy Digital Downloads needs to update the order items table. <a href="%s">Start the upgrade.</a>', 'templify-recurring' ) . '</p></div>',
...
function edd_recurring_v24_migrate_subscriptions() {
	...
	if ( ! current_user_can( 'manage_shop_settings' ) ) {
		wp_die( __( 'You do not have permission to do shop upgrades', 'templify-recurring' ), __( 'Error', 'templify-recurring' ), array( 'response' => 403 ) );
	}
						...
						if ( ! empty( $edd_sub->profile_id ) ) {
							if ( true === $log_only ) {
								...
								$edd_sub->customer->add_note( sprintf(
									__( 'Customer\'s subscription #%d was improperly attributed to the incorrect Stripe customer record. It was automatically cancelled to prevent incorrect renewal payments.', 'templify-recurring' ),
									$edd_sub->id
								) );
							}
						}
...
function edd_upgrade_render_recurring_add_price_id_column() {
	...
	if ( $migration_complete ) : ?>
		<div id="edd-sl-migration-complete" class="notice notice-success">
			<p>
				<?php _e( '<strong>Migration complete:</strong> You have already completed the subscription Price ID upgrade.', 'templify-recurring' ); ?>
			</p>
		</div>
		<?php return; ?>
	<?php endif; ?>
	...
	<div id="edd-migration-ready" class="notice notice-success" style="display: none;">
		<p>
			<?php _e( '<strong>Database Upgrade Complete:</strong> All database upgrades have been completed.', 'templify-recurring' ); ?>
			<br /><br />
			<?php _e( 'You may now leave this page.', 'templify-recurring' ); ?>
		</p>
	</div>

	<div id="edd-migration-nav-warn" class="notice notice-info">
		<p>
			<?php _e( '<strong>Important:</strong> Please leave this screen open and do not navigate away until the process completes.', 'templify-recurring' ); ?>
		</p>
	</div>
	...
	<div class="metabox-holder">
		<div class="postbox">
			<h2 class="hndle">
				<span><?php _e( 'Update subscription records', 'templify-recurring' ); ?></span>
				<span class="dashicons dashicons-yes"></span>
			</h2>
			<div class="inside update-subscription-records-control">
				<p>
					<?php _e( 'This update will add the price ID of any variably priced subscription to the subscription record in the database.', 'templify-recurring' ); ?>
				</p>
			...
			<span class="step-instructions-wrapper">
				...
				<?php if ( ! $migration_complete ) : ?>
					<span class="edd-migration allowed">
						<input type="submit" id="add-price-ids-submit" value="<?php _e( 'Update Subscriptions', 'templify-recurring' ); ?>" class="button-primary"/>
					</span>
				<?php else: ?>
					<input type="submit" disabled="disabled" id="migrate-logs-submit" value="<?php _e( 'Update Subscriptions', 'templify-recurring' ); ?>" class="button-secondary"/>
					&mdash; <?php _e( 'Subscription records have already been updated.', 'templify-recurring' ); ?>
				<?php endif; ?>
...
function edd_upgrade_render_recurring_update_price_id_column() {
	...
	<div class="metabox-holder">
		<div class="postbox">
			...
			<div class="inside update-subscription-records-control">
				<p>
					<?php _e( 'This update will update the price ID of any variably priced subscription to the subscription record in the database.', 'templify-recurring' ); ?>
				</p>
				<form method="post" id="edd-add-price-id-column-subs-form" class="edd-export-form edd-import-export-form">
			<span class="step-instructions-wrapper">
				...
				<?php if ( ! $migration_complete ) : ?>
					<span class="edd-migration allowed">
						<input type="submit" id="update-price-ids-submit" value="<?php _e( 'Update Subscriptions', 'templify-recurring' ); ?>" class="button-primary"/>
					</span>
				<?php else: ?>
					<input type="submit" disabled="disabled" id="migrate-logs-submit" value="<?php _e( 'Update Subscriptions', 'templify-recurring' ); ?>" class="button-secondary"/>
					&mdash; <?php _e( 'Subscription records have already been updated.', 'templify-recurring' ); ?>
...
function edd_recurring_fix_edd_subscriber_role() {
	...
	if ( $subscriber_role_exists ) {
		add_role( 'edd_subscriber', __( 'EDD Subscriber', 'templify-recurring' ), array( 'read' => true ) );
	}
