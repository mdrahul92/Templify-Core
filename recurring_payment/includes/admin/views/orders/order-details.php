...
<div class="edd-recurring-subscription-details">
	...
	<div class="edd-recurring-subscription-details--status">
		...
		printf(
			...
			esc_html__( 'Status: %s', 'templify-recurring' ),
	...
	if ( ! empty( $sub->expiration ) && in_array( $sub->status, array( 'active', 'trialling' ), true ) ) {
		...
		<div class="edd-recurring-subscription-details--renewal">
			...
			printf(
				...
				esc_html__( 'Renewing On: %s', 'templify-recurring' ),
...
?>
<div>
	<h3><?php esc_html_e( 'Associated Orders', 'templify-recurring' ); ?>:</h3>
	<table class="wp-list-table widefat striped">
		<thead>
			<tr>
				<th class="column-primary"><?php esc_html_e( 'Order', 'templify-recurring' ); ?></th>
				<th><?php esc_html_e( 'Total', 'templify-recurring' ); ?></th>
				...
				<tr>
					<td class="column-primary">
						...
						printf(
							...
							(int) $payment->ID === (int) $order->id ? '(' . esc_html__( 'this order', 'templify-recurring' ) . ')' : ''
						);
						?>
						<button type="button" class="toggle-row">
							<span class="screen-reader-text"><?php esc_html_e( 'Show order details', 'templify-recurring' ); ?></span>
						</button>
					</td>
					<td data-colname="<?php esc_html_e( 'Total', 'templify-recurring' ); ?>"><?php echo edd_currency_filter( edd_format_amount( $payment->total ), $payment->currency ); ?></td>
					<td data-colname="<?php esc_html_e( 'Date', 'templify-recurring' ); ?>"><?php echo esc_html( date_i18n( get_option( 'date_format' ), strtotime( $payment->date ) ) ); ?></td>
					<td data-colname="<?php esc_html_e( 'Status', 'templify-recurring' ); ?>"><?php echo esc_html( $payment->status_nicename ); ?></td>
				</tr>
				<?php
			}
			?>
