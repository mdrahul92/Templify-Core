...
add_action( 'edd_reports_tab_export_content_bottom', function () {
	...
	<div class="postbox edd-export-payment-history">
		<h2 class="hndle"><span><?php esc_html_e( 'Export Subscriptions', 'templify-recurring' ); ?></span></h2>
		<div class="inside">
			<p><?php esc_html_e( 'Download a CSV of all subscriptions. The datepickers can be used to filter by subscription start date; only subscriptions started within that range will be included.', 'templify-recurring' ); ?></p>
			...
			<form id="edd-export-subscriptions" class="edd-export-form edd-import-export-form" method="post">
				<label for="edd-subscriptions-export-product" class="screen-reader-text">
					<?php esc_html_e( 'Select Product', 'templify-recurring' ); ?>
				</label>
				<?php
				echo EDD()->html->product_dropdown( array(
					...
					'selected'        => 'all',
					'show_option_all' => sprintf( __( 'All %s', 'templify-recurring' ), edd_get_label_plural() ),
					'chosen'          => true,
					/* translators: the plural post type label */
					'placeholder'     => sprintf( __( 'All %s', 'templify-recurring' ), edd_get_label_plural() ),
				) );
				?>

				<span class="edd-from-to-wrapper">
					<label for="edd-subscriptions-export-start" class="screen-reader-text">
						<?php esc_html_e( 'Set start date', 'templify-recurring' ); ?>
					</label>
					<?php
					echo EDD()->html->date_field( array(
						'id'          => 'edd-subscriptions-export-start',
						'name'        => 'start',
						'placeholder' => __( 'Choose start date', 'templify-recurring' )
					) );
					?>

					<label for="edd-subscriptions-export-end" class="screen-reader-text">
						<?php esc_html_e( 'Set end date', 'templify-recurring' ); ?>
					</label>
					<?php
					echo EDD()->html->date_field( array(
						'id'          => 'edd-subscriptions-export-end',
						'name'        => 'end',
						'placeholder' => __( 'Choose end date', 'templify-recurring' )
					) );
					?>
				</span>

				<label for="edd-subscriptions-status" class="screen-reader-text">
					<?php esc_html_e( 'Filter by status', 'templify-recurring' ); ?>
				</label>
				<?php
				...
				<button type="submit" class="button button-secondary"><?php esc_html_e( 'Generate CSV', 'templify-recurring' ); ?></button>
			</form>
