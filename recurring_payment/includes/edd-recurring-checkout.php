...
class EDD_Recurring_Checkout {
	...
	public function force_login_fields() {
		if ( isset( $_GET['edd-recurring-login'] ) && '1' === $_GET['edd-recurring-login'] ) {
			...
			<div class="edd-alert edd-alert-info">
				<p><?php _e( 'An account was detected for your email. Please log in to continue your purchase.', 'templify-recurring' ); ?></p>
				<p>
					<a href="<?php echo wp_lostpassword_url(); ?>" title="<?php _e( 'Lost Password', 'templify-recurring' ); ?>">
						<?php _e( 'Lost Password?', 'templify-recurring' ); ?>
					</a>
				</p>
			</div>
			<?php
	...
	public function display_failed_subscriptions( $payment, $receipt_args ) {
		...
		<div class="eddr-failed-subscription-notice">
			<div class="edd-alert edd-alert-warn">
				<p>
					<strong><?php _e( 'Notice', 'templify-recurring' ); ?>:</strong> <?php _e( 'Your purchase is completed, but we encountered an issue while processing payments for the following items', 'templify-recurring' ); ?>:
				</p>
				...
				<p>
					<?php _e( 'The above items were removed from the purchase and you were not charged for them. You can attempt to repurchase them at your convenience. All other items were purchased successfully.', 'templify-recurring' ); ?>
				</p>
				<p>
					<form id="edd-recurring-add-failed" class="edd-form" method="post">
						<?php foreach ( $failed_subscriptions as $key => $subscription ) : ?>
							...
							<?php endif; ?>
						<?php endforeach; ?>
						<input type="submit" class="button" name="edd_recurring_add_failed" value="<?php _e( 'Try Again', 'templify-recurring' ); ?>"/>
	...
	public function show_single_signup_fee_notice( $download_id, $args ) {
		...
		$text = edd_get_option( 'recurring_signup_fee_label', __( 'signup fee', 'templify-recurring' ) );
		?>
		<p class="eddr-notice eddr-signup-fee-notice">
			<em><?php printf( __( 'With %s %s', 'templify-recurring' ), $formatted_price, $text ); ?></em>
		</p>
		<?php
	...
	public function free_trial_total() {
		...
		$message  = __( 'Your account will be automatically charged when the free trial is completed.', 'templify-recurring' );
		...
		if ( 1 === count( $contents ) ) {
			...
			if ( isset( $subscription_data['recurring_amount'] ) ) {
				$message = sprintf(
					...
					__( 'Your account will be automatically charged %s when the free trial is completed.', 'templify-recurring' ),
		...
		<p id="edd_final_total_wrap">
			<strong><?php esc_html_e( 'Total Due Today:', 'templify-recurring' ); ?></strong>
		...
		if ( ! empty( $download_ids ) ) {
			foreach( $download_ids as $download_id ) {
				...
				if ( edd_recurring()->has_trialed( $download_id, $email ) ) {
					...
					$message .= sprintf(
						__( 'You have already used the free trial for <strong>%s</strong>. Please log into your account to complete the purchase. You will be charged immediately.', 'templify-recurring' ),
