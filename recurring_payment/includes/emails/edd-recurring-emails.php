...
class EDD_Recurring_Emails {
	...
	public function send_reminder( $subscription_id = 0, $notice_id = 0 ) {
		...
		$subject    = ! empty( $notice['subject'] ) ? $notice['subject'] : __( 'Your Subscription is About to Renew or Expire', 'templify-recurring' );
		$subject    = $this->filter_reminder_template_tags( $subject, $subscription_id );
		...
		$log_id = wp_insert_post(
			array(
				'post_title'   => __( 'LOG - Subscription Reminder Notice Sent', 'templify-recurring' ),
	...
	public function filter_reminder_template_tags( $text = '', $subscription_id = 0 ) {
		...
		$text = str_replace( '{subscription_period}', esc_html( __( $this->subscription->period, 'templify-recurring' ) ), $text );
		$text = str_replace(
