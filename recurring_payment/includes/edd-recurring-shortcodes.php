...
class EDD_Recurring_Shortcodes {
	...
	public function cancel_link( $atts, $content = null ) {
		...
		$link       = sprintf(
			$link,
			$cancel_url,
			__( 'Cancel your subscription', 'templify-recurring' ),
			empty( $atts['text'] ) ? __( 'Cancel Subscription', 'templify-recurring' ) : esc_html( $atts['text'] )
		);
	...
	private function verify_profile_update_action( $user_id ) {
		...
		if ( false === $passed_nonce || ! isset( $_POST['_wp_http_referer'] ) ) {
			wp_die( __( 'Invalid Payment Update', 'templify-recurring' ) );
		}

		$verified = wp_verify_nonce( $passed_nonce, 'update-payment' );

		if ( 1 !== $verified || (int) $user_id !== (int) get_current_user_id() ) {
			wp_die( __( 'Unable to verify payment update. Please try again later.', 'templify-recurring' ) );
		}
