...
class EDD_Recurring_Gateway {
	...
	public function validate_fields( $data, $posted ) {
		...
		if( true ) {
			edd_set_error( 'error_id_here', __( 'Error message here', 'templify-recurring' ) );
		}
	...
	public function process_checkout( $purchase_data ) {
		...
		if ( ! wp_verify_nonce( $purchase_data['gateway_nonce'], 'edd-gateway' ) ) {
			wp_die( __( 'Nonce verification has failed', 'templify-recurring' ), __( 'Error', 'templify-recurring' ), array( 'response' => 403 ) );
		}

		if ( $purchase_data['user_info']['id'] < 1 && ! class_exists( 'EDD_Auto_Register' ) ) {
			edd_set_error( 'edd_recurring_logged_in', __( 'You must log in or create an account to purchase a subscription', 'templify-recurring' ) );
		}
		...
		if ( ! is_user_logged_in() ) {
			edd_set_error( 'edd_recurring_login', __( 'You must be logged in to purchase a subscription.', 'templify-recurring' ) );
		...
		if ( ! empty( $this->failed_subscriptions ) ) {
			...
			if ( count( $this->failed_subscriptions ) === $initial_subscription_count ) {
				if ( ! empty( $failed_sub['error'] ) ) {
					...
					edd_set_error( 'recurring-all-subscriptions-failed', __( 'There was an error processing your order. Please contact support.', 'templify-recurring' ) );
				}
			}
	...
	public function setup_customer_subscriber() {
		...
		if ( empty( $subscriber->id ) ) {
			if ( empty( $user_id ) && email_exists( $email ) ) {
				edd_set_error( 'existing_email', __( 'This purchase cannot be completed with this email address. If you already have an account, please log in and try again.', 'templify-recurring' ) );
				return false;
			}
			$name = '';
	...
	public function record_signup() {
		...
		if ( ! empty( $this->failed_subscriptions ) ) {
			...
			foreach ( $this->failed_subscriptions as $failed_subscription ) {
				$note = sprintf( __( 'Failed creating subscription for %s. Gateway returned: %s', 'templify-recurring' ), $failed_subscription['subscription']['name'], $failed_subscription['error'] );
	...
	public function checkout_errors( $data, $posted ) {
		...
		if ( edd_recurring()->cart_is_mixed_with_trials() ) {
			edd_set_error( 'edd_recurring_mixed_trials_cart', __( 'Free trials and non-trials may not be purchased at the same time. Please purchase each separately.', 'templify-recurring' ) );
		}
		...
		if ( $show_mixed_error ) {
			...
			if ( ! isset( $enabled_gateways['stripe'] ) ) {
				...
				if ( ! current_user_can( 'manage_shop_settings' ) ) {
					edd_set_error( 'edd_recurring_mixed_cart', __( 'Subscriptions and non-subscriptions may not be purchased at the same time. Please purchase each separately.', 'templify-recurring' ) );
				...
				// Alert shop managers that Stripe supports mixed carts.
					...
					edd_set_error(
						...
						wp_kses(
							wpautop(
								'<em>' . __( 'This message is showing because you are a shop manager', 'templify-recurring' ) . '</em>'
							) .
							wpautop(
								sprintf(
									/** translators: %1$s Opening anchor tag, do not translate. %2$s Closing anchor tag, do not translate. */
									__( 'Your active payment gateways do not support mixed carts. The %1$sStripe Payment Gateway%2$s allows customers to purchase carts containing both recurring subscriptions and one-time charges at the same time.', 'templify-recurring' ),
			...
			// Show an error to switch to the Stripe gateway to complete purchase.
				...
				edd_set_error(
					...
					wp_kses(
						sprintf(
							__( 'Sorry, purchasing a subscription and non-subscription product is only supported when paying by credit card. %1$sSwitch to this payment method%2$s.', 'templify-recurring' ),
	...
	public function process_payment_method_update( $user_id, $subscription_id, $verified ) {
		...
		if ( 1 !== $verified ) {
			wp_die( __( 'Unable to verify payment update.', 'templify-recurring' ) );
		}

		if ( ! is_user_logged_in() ) {
			wp_die( __( 'You must be logged in to update a payment method.', 'templify-recurring' ) );
		}
	...
	public function process_cancellation( $data ) {
		...
		if ( ! $subscription->current_user_can() ) {
			wp_die( esc_html__( 'You do not have permission to cancel this subscription.', 'templify-recurring' ), esc_html__( 'Error', 'templify-recurring' ), array( 'response' => 403 ) );
		}
		...
		try {
			...
			wp_die( $e->getMessage(), __( 'Error', 'templify-recurring' ), array( 'response' => 403 ) );
		}
	...
	public function process_reactivation( $data ) {
		...
		if ( ! $subscription->can_reactivate() ) {
			wp_die( __( 'This subscription cannot be reactivated', 'templify-recurring' ), __( 'Error', 'templify-recurring' ), array( 'response' => 403 ) );
		}
		...
		try {
			...
			$note = sprintf( __( 'Subscription reactivated by %s', 'templify-recurring' ), $user );
