		...
		switch ( strtolower( $this->event->resource->state ) ) {
			case 'declined' :
				...
				$payment->add_note( sprintf(
					__( 'PayPal payment declined. Details: %s', 'templify-recurring' ),
					( ! empty( $this->event->resource->status_details ) ? json_encode( $this->event->resource->status_details ) : __( 'n/a', 'templify-recurring' ) )
				) );
				$payment->save();
			...
			case 'pending' :
				if ( ! empty( $this->event->resource->capture_status_details->reason ) ) {
					...
					$payment->add_note( __( 'Payment still processing in PayPal.', 'templify-recurring' ) . ' ' . $reason );
				}
			...
			case 'completed' :
				try {
					...
					$note = sprintf(
						...
						__( 'Payment failed. Error message: %1$s. Payment data: %2$s.', 'templify-recurring' ),
						$e->getMessage(),
		...
		if ( 'completed' === strtolower( $this->event->resource->state ) ) {
			...
			if ( ! empty( $this->event->resource->amount->currency ) && strtoupper( $subscription_currency ) !== strtoupper( $this->event->resource->amount->currency ) ) {
				...
				/* Translators: %s - currency code */
					__( 'Renewal payment processing failed due to invalid currency. PayPal currency: %s', 'templify-recurring' ),
