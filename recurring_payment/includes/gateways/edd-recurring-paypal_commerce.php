...
class EDD_Recurring_PayPal_Commerce extends EDD_Recurring_Gateway {
	...
	public function validate_fields( $data, $posted ) {
		...
		if ( 'INR' === edd_get_currency() ) {
			edd_set_error( 'unsupported_currency', __( 'PayPal subscriptions cannot be made with Indian Rupees.', 'templify-recurring' ) );
		}
	}
	...
	protected function handle_errors( $errors = array() ) {
		if ( ! is_array( $errors ) || empty( $errors ) ) {
			$errors = array(
				'paypal-error' => __( 'An unexpected error occurred.', 'templify-recurring' )
			);
		}
	...
	public function create_payment_profiles() {
		...
		if ( ! \EDD\Gateways\PayPal\has_rest_api_connection() ) {
			edd_record_gateway_error(
				__( 'PayPal Gateway Error', 'templify-recurring' ),
				__( 'Missing PayPal Commerce credentials.', 'templify-recurring' )
			);

			$error_message = current_user_can( 'manage_options' )
				? __( 'Please connect your PayPal account in the gateway settings.', 'templify-recurring' )
				: __( 'Unexpected authentication error. Please contact a site administrator.', 'templify-recurring' );
		...
		try {
			try {
					...
					if ( empty( $subscription['has_trial'] ) ) {
						...
						if ( $sub_initial_amount !== $cart_total ) {
							...
							throw new Gateway_Exception(
								__( 'The cart total does not match the subscription amount.', 'templify-recurring' ),
								403,
								$message
							);
						}
				...
				throw new Gateway_Exception( __( 'An authentication error occurred. Please try again.', 'templify-recurring' ), $e->getCode(), $e->getMessage() );
			} catch ( API_Exception $e ) {
				throw new Gateway_Exception( __( 'An error occurred while communicating with PayPal. Please try again.', 'templify-recurring' ), $e->getCode(), $e->getMessage() );
			}
	...
	protected function get_or_create_paypal_product( $download_id ) {
		...
		if ( empty( $download->ID ) ) {
			throw new Gateway_Exception(
				__( 'An unexpected error has occurred. Please try again.', 'templify-recurring' ),
				500,
	...
	public function cancel( $subscription, $valid ) {
		...
		try {
			...
			$api->make_request( sprintf( 'v1/billing/subscriptions/' . urlencode( $subscription->profile_id ) . '/cancel' ), array(
				'reason' => esc_html__( 'Customer requested cancellation.', 'templify-recurring' )
			) );
			...
			$subscription->add_note( sprintf(
				__( 'Failed to cancel subscription in PayPal. Message: %s', 'templify-recurring' ),
	...
	public function get_subscription_details( EDD_Subscription $subscription ) {
		...
		try {
			if ( empty( $subscription->profile_id ) ) {
				throw new \Exception( __( 'Missing profile ID.', 'templify-recurring' ) );
			}
			...
			if ( 200 !== $api->last_response_code ) {
				...
				/* Translators: %d - the HTTP response code */
					__( 'Unexpected HTTP response code: %d.', 'templify-recurring' ),
					$api->last_response_code
				) );
			}
			...
			if ( empty( $response->id ) ) {
				...
				/* Translators: %s - response from PayPal */
					__( 'PayPal response missing subscription ID. Response: %s', 'templify-recurring' ),
					json_encode( $response )
				) );
			}
			...
			$details['error'] = new WP_Error( 'authentication_exception', __( 'An authentication exception occurred.', 'templify-recurring' ) );
		} catch ( API_Exception $e ) {
			$details['error'] = new WP_Error( 'api_exception', sprintf(
			/* Translators: %d - HTTP response code; %s - Error message; %s */
				__( 'An error occurred in the response from PayPal. HTTP code: %d; Message: %s', 'templify-recurring' ),
			...
			/* Translators: %s - error message */
				__( 'An unexpected error occurred. Message: %s', 'templify-recurring' ),
				$e->getMessage()
			) );
		}
		...
		try {
			...
			if ( ! empty( $timestamp ) && ! empty( $token ) ) {
				if ( !\EDD\Utils\Tokenizer::is_token_valid( $token, $timestamp ) ) {
					throw new Gateway_Exception(
						__('A validation error occurred, but your payment may have gone through. Please contact the site administrator.', 'templify-recurring'),
						403,
			...
			$default_error_message = __( 'An unexpected error occurred, but your payment may have gone through. Please contact the site administrator.', 'templify-recurring' );
			...
			try {
				...
				if ( $attempt_number < 5 ) {
					...
					wp_send_json_error( array_merge( $response, array(
						'retry'         => false,
						'error_message' => __( 'Exceeded maximum attempts.', 'templify-recurring' ),
					) ) );
				}
			}
		} catch ( Gateway_Exception $e ) {
	...
	public static function capture_status_to_note( $reason ) {
		switch ( strtoupper( $reason ) ) {
			case 'BUYER_COMPLAINT' :
				return __( 'The payer has initiated a dispute for this payment.', 'templify-recurring' );
			case 'CHARGEBACK' :
				return __( 'The captured funds were reversed in response to the payer disputing this payment.', 'templify-recurring' );
			case 'ECHECK' :
				return __( 'The payment was made using an eCheck that has not yet cleared.', 'templify-recurring' );
			case 'INTERNATIONAL_WITHDRAWAL' :
				return __( 'Visit your PayPal account to accept or deny this payment in your "Account Overview."', 'templify-recurring' );
			case 'PENDING_REVIEW' :
				return __( 'This payment is pending manual review.', 'templify-recurring' );
			case 'RECEIVING_PREFERENCE_MANDATES_MANUAL_ACTION' :
				return __( 'You have not yet set up appropriate receiving preferences for your account. Visit your PayPal account for more information.', 'templify-recurring' );
			case 'REFUNDED' :
				return __( 'The captured funds were refunded.', 'templify-recurring' );
			case 'TRANSACTION_APPROVED_AWAITING_FUNDING' :
				return __( 'Waiting for the payer to send the funds for this payment.', 'templify-recurring' );
			case 'UNILATERAL' :
				return __( 'You do not appear to have a PayPal account. Please contact PayPal for more information.', 'templify-recurring' );
			case 'VERIFICATION_REQUIRED' :
				return __( 'Your PayPal account has not yet been verified. Check your account or contact PayPal for more information.', 'templify-recurring' );
			default :
				return __( 'No reason has been provided. For more information about this payment, visit your PayPal account or contact PayPal directly.', 'templify-recurring' );
		}
	}
}
