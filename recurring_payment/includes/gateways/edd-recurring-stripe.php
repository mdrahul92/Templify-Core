...
class EDD_Recurring_Stripe extends EDD_Recurring_Gateway {
	...
	public function init() {
		$this->id            = 'stripe';
		$this->friendly_name = __( 'Stripe', 'templify-recurring' );
		$this->supports      = array(
			'mixed_cart',
		);
		...
		if (
			...
			echo wpautop( wp_kses(
				sprintf(
					...
					__( '%1$sCredit card payments with Stripe are currently disabled.%2$s', 'templify-recurring' ),
					'<strong>',
					'</strong>'
				)
				. '<br />' .
				sprintf(
					...
					__( 'To continue accepting recurring credit card payments with Stripe please update the Stripe Payment Gateway extension to version %1$s2.7%2$s.', 'templify-recurring' ),
					'<code>',
					'</code>'
				),
				array(
	...
	public function require_login_for_existing_users() {
		...
		if ( email_exists( $purchase_data['user_email'] ) ) {
			...
			if ( ! is_user_logged_in() ) {
				...
				throw new \Exception( sprintf( __( 'A customer account for %1$s already exists. Please log in to complete your purchase.', 'templify-recurring' ), esc_html( $purchase_data['user_email'] ) ) );
			}
		}
	}
	...
	public function setup_stripe_api() {
		_doing_it_wrong(
			__METHOD__,
			__( 'Use edds_api_request() to make Stripe API requests.', 'templify-recurring' ),
			'2.10.0'
		);
	}
		...
		if ( $customer_currency !== $store_currency ) {
			throw new \Exception(
				sprintf(
					...
					__( 'Unable to complete your purchase. Your order must be completed in %1$s.', 'templify-recurring' ),
	...
	public function create_stripe_subscriptions( $intent ) {
		...
		foreach ( $this->failed_subscriptions as $failed_subscription ) {
			...
			$this->payment->add_note( sprintf( __( 'Failed creating subscription for %s. Gateway returned: %s', 'templify-recurring' ), $failed_subscription['subscription']['name'], $failed_subscription['error'] ) );
	...
	public function capture_payment_intent( $intent ) {
		...
		if ( 0 === intval( $amount ) ) {
			...
			$payment->add_note( esc_html__( 'PaymentIntent cancelled because there is nothing to collect.', 'templify-recurring' ) );
		...
		if ( isset( $event_json->id ) ) {
			...
			switch ( $event->type ) :
				...
				case 'invoice.payment_failed' :
					...
					$subscription->add_note( sprintf( __( 'Failing invoice URL: %s', 'templify-recurring' ), $event->data->object->hosted_invoice_url ) );
	...
	public function cancel( $subscription, $valid ) {
		...
		try {
			...
			$subscription->add_note( sprintf( esc_html__( 'Attempted cancellation but was unable. Message was %s.', 'templify-recurring' ), wp_json_encode( $e->getMessage() ) ) );
			return false;
		}
		...
		try {
			...
			if ( empty( $sub->cancel_at_period_end ) || in_array( $sub->status, array( 'canceled', 'incomplete', 'incomplete_expired' ), true ) ) {
				...
				if ( 'incomplete' === $stripe_sub->status ) {
					$subscription->add_note( esc_html__( 'Subscription reactivation requires payment by customer and will be cancelled in 24 hours if no action is taken.', 'templify-recurring' ) );
				}
			...
			wp_die( esc_html( $e->getMessage() ), esc_html( __( 'Error', 'templify-recurring' ) ), array( 'response' => 403 ) );
		}
				...
				try {
					...
					$payment->add_note(
						sprintf(
							...
							__( 'Charge refunded in Stripe. Refund ID %s', 'templify-recurring' ),
							$refund->id
						)
					);
				} catch ( \Exception $e ) {
					wp_die( $e->getMessage(), __( 'Error', 'templify-recurring' ), array( 'response' => 400 ) );
				}
	...
	public function update_payment_method_form( $subscription ) {
		...
		wp_localize_script(
			...
			array(
				'i18n' => array(
					'loading' => esc_html__( 'Please Wait…', 'templify-recurring' ),
				),
			)
		);
	...
	public function update_subscription_payment_method() {
		...
		if ( ! $nonce || ! wp_verify_nonce( $nonce, 'update-payment' ) ) {
			return wp_send_json_error( array(
				'message' => esc_html__( 'Invalid request. Please try again', 'templify-recurring' ),
			) );
		}

		if ( ! $subscription_id || ! $payment_method_id ) {
			return wp_send_json_error( array(
				'message' => esc_html__( 'Unable to locate Subscription. Please try again', 'templify-recurring' ),
			) );
		}
		...
		try {
			...
			return wp_send_json_success( array(
				'message'      => esc_html__( 'Payment method updated.', 'templify-recurring' ),
				'subscription' => $subscription,
			) );
		} catch( \Exception $e ) {
			return wp_send_json_error( array(
