...
class EDD_Recurring_PayPal extends EDD_Recurring_Gateway {
	...
	public function validate_fields( $data, $posted ) {
		...
		if( ! edd_get_option( 'paypal_email', false ) ) {
			...
			edd_set_error( 'edd_recurring_paypal_email_missing', __( 'Please enter your PayPal email address.', 'templify-recurring' ) );

		}

		if( count( edd_get_cart_contents() ) > 1 && ! $this->can_purchase_multiple_subs() ) {

			edd_set_error( 'subscription_invalid', __( 'Only one subscription may be purchased through PayPal per checkout.', 'templify-recurring') );
	...
	public function process_paypal_subscr_signup( $ipn_data ) {
		...
		edd_insert_payment_note( $parent_payment_id, sprintf( __( 'PayPal Subscription ID: %s', 'templify-recurring' ) , $ipn_data['subscr_id'] ) );
		...
		if( $amount_received < $initial_amount ) {
			...
			$payment->add_note( __( 'Payment failed due to invalid initial amount in PayPal Recurring IPN.', 'templify-recurring' ) );
			$payment->save();

			edd_record_gateway_error( __( 'IPN Error', 'templify-recurring' ), sprintf( __( 'Invalid initial payment amount in IPN subscr_signup response. IPN data: %s', 'templify-recurring' ), json_encode( $ipn_data ) ), $payment->ID );
		...
		if( $amount_to_receive < $recurring_amount ) {
			...
			$payment         = edd_get_payment( $subscription->parent_payment_id );
			$payment->status = 'failed';
			$payment->add_note( __( 'Payment failed due to invalid recurring amount in PayPal Recurring IPN.', 'templify-recurring' ) );
			$payment->save();

			edd_record_gateway_error( __( 'IPN Error', 'templify-recurring' ), sprintf( __( 'Invalid recurring payment amount in IPN subscr_signup response. IPN data: %s', 'templify-recurring' ), json_encode( $ipn_data ) ), $payment->ID );
	...
	public function process_paypal_subscr_payment( $ipn_data ) {
		...
		if( $probably_is_initial_ipn && ( ! $transaction_id || $transaction_id == $subscription->parent_payment_id ) ) {
			...
			if ( 'failed' === strtolower( $ipn_data['payment_status'] ) ) {
				$payment->status = 'failed';
				$payment->add_note( __( 'Payment failed at PayPal.', 'templify-recurring' ) );
				$subscription->add_note( __( 'Payment processing failed at PayPal.', 'templify-recurring' ) );
				$payment->save();
			...
			if( $paid_amount < $initial_amount ) {
				...
				$payment->status = 'failed';
				$payment->add_note( __( 'Payment failed due to invalid amount in PayPal Recurring IPN.', 'templify-recurring' ) );
				$payment->save();

				edd_record_gateway_error( __( 'IPN Error', 'templify-recurring' ), sprintf( __( 'Invalid payment amount in IPN subscr_payment response. IPN data: %s', 'templify-recurring' ), json_encode( $ipn_data ) ), $payment->ID );
			...
			if ( 'pending' == $payment_status && isset( $ipn_data['pending_reason'] ) ) {
				...
				switch( strtolower( $ipn_data['pending_reason'] ) ) {
					...
					case 'echeck' :
						...
						$note = __( 'Payment made via eCheck and will clear automatically in 5-8 days.', 'templify-recurring' );
						break;

					case 'address' :

						$note = __( 'Payment requires a confirmed customer address and must be accepted manually through PayPal.', 'templify-recurring' );

						break;

					case 'intl' :

						$note = __( 'Payment must be accepted manually through PayPal due to international account regulations.', 'templify-recurring' );

						break;

					case 'multi_currency' :

						$note = __( 'Payment received in non-shop currency and must be accepted manually through PayPal.', 'templify-recurring' );
					...
					case 'regulatory_review' :
						...
						$note = __( 'Payment is being reviewed by PayPal staff as high-risk or in possible violation of government regulations.', 'templify-recurring' );

						break;

					case 'delayed_disbursement' :

						$note = __( 'The transaction has been approved and is currently awaiting funding from the bank. This typically takes less than 48 hrs.', 'templify-recurring' );

						break;

					case 'unilateral' :

						$note = __( 'Payment was sent to non-confirmed or non-registered email address.', 'templify-recurring' );

						break;

					case 'upgrade' :

						$note = __( 'PayPal account must be upgraded before this payment can be accepted.', 'templify-recurring' );

						break;

					case 'verify' :

						$note = __( 'PayPal account is not verified. Verify account in order to accept this payment.', 'templify-recurring' );
					...
					default :
						...
						$note = __( 'Payment is pending for unknown reasons. Contact PayPal support for assistance.', 'templify-recurring' );
		...
		if ( 'failed' === strtolower( $ipn_data['payment_status'] ) ) {
			...
			$subscription->update( array( 'status' => 'failing' ) );
			$subscription->add_note( __( 'Renewal payment processing failed at PayPal.', 'templify-recurring' ) );
			return;
		}
		...
		if( ! empty( $sub_currency ) && $currency_code != strtolower( $sub_currency ) ) {
			...
			$subscription->add_note( sprintf( __( 'Renewal payment processing failed due to invalid currency. Currency detected in renewal payment: %s', 'templify-recurring' ), $currency_code ) );
			edd_record_gateway_error( __( 'IPN Error', 'templify-recurring' ), sprintf( __( 'Invalid currency in IPN response. IPN data: ', 'templify-recurring' ), json_encode( $ipn_data ) ) );
			return;
		}
	...
	public function validate_paypal_recurring_download( $post_id = 0 ) {
		...
		$message = __( 'PayPal Standard requires recurring times to be set to 0 for indefinite subscriptions or a minimum value of 2 and a maximum value of 52 for limited subscriptions.', 'templify-recurring' );
		...
		if ( edd_has_variable_prices( $post_id ) ) {
			...
			foreach ( $prices as $price_id => $price ) {
				if ( EDD_Recurring()->is_price_recurring( $post_id, $price_id ) ) {
					...
					if ( $time == 1  || $time >= 53 ) {
						wp_die( $message, __( 'Error', 'templify-recurring' ), array( 'response' => 400 ) );
					}
				}
			}
		} else {
			...
			case 'revoked' :
				...
				foreach( $subs as $subscription ) {
					...
					if ( 'cancelled' !== $subscription->status ) {
						...
						$subscription->cancel();
						$payment->add_note( sprintf( __( 'Subscription %d cancelled.', 'templify-recurring' ), $subscription->id ) );
	...
	public function retry( $result, $subscription ) {
		...
		$args = array(
			...
			'PROFILEID' => $subscription->profile_id,
			'NOTE'      => __( 'Retry initiated from EDD Recurring', 'templify-recurring' )
		);
	...
	public function get_subscription_details( EDD_Subscription $subscription ) {
		...
		if( empty( $creds['username'] ) || empty( $creds['password'] ) || empty( $creds['password'] ) ) {
			$ret['error'] = new WP_Error( 'missing_api_credentials', __( 'Missing PayPal API credentials', 'templify-recurring' ) );
		}

		if( ! $subscription->id > 0 ) {

			$ret['error'] = new WP_Error( 'invalid_subscription', __( 'Invalid subscription object supplied', 'templify-recurring' ) );
			...
			if( ! empty( $subscription->profile_id ) ) {
				...
				if ( is_wp_error( $request ) ) {
					...
					if( empty( $code ) || 200 !== (int) $code ) {
						$ret['error'] = new WP_Error( 'paypal_api_error', sprintf( __( 'Non 200 response code. Response code was: %s', 'templify-recurring' ), $code ) );
					}

					if( empty( $message ) || 'OK' !== $message ) {
						$ret['error'] = new WP_Error( 'paypal_api_error', sprintf( __( 'Response message not okay. Response message was: %s', 'templify-recurring' ), $message ) );
					}
				...
				$ret['error'] = new WP_Error( 'missing_profile_id', __( 'No profile_id set on subscription object', 'templify-recurring' ) );
