...
class EDD_Recurring_2Checkout extends EDD_Recurring_Gateway {
	...
	public function init() {
		...
		$this->id            = '2checkout';
		$this->friendly_name = __( '2Checkout', 'templify-recurring' );
	...
	public function validate_fields( $data, $posted ) {
		...
		if( empty( $this->credentials['tco_secret_word'] ) || empty( $this->credentials['tco_account_number'] ) ) {
			...
			edd_set_error( 'missing_account_number', __( 'You must enter your account number and secret word in settings', 'templify-recurring' ) );

		}

		if( count( edd_get_cart_contents() ) > 1 && ! $this->can_purchase_multiple_subs() ) {

			edd_set_error( 'subscription_invalid', __( 'Only one subscription may be purchased at a time through 2Checkout.', 'templify-recurring') );
	...
	public function create_payment_profiles() {
		...
		foreach( $this->subscriptions as $key => $subscription ) {
			...
			if( ! empty( $subscription['has_trial'] ) ) {
				edd_set_error( 'free_trial_not_supported', __( 'Free trials are not supported by 2Checkout.', 'templify-recurring' ) );
				return;
			}
			...
			switch( strtoupper( $_POST['message_type'] ) ) {
				...
				case 'FRAUD_STATUS_CHANGED':
					...
					switch( $fraud_status ) {
						...
						case 'fail':
							...
							$initial_payment->add_note( __( '2Checkout fraud review failed.', 'templify-recurring' ) );
						...
						case 'wait':
							...
							$initial_payment->add_note( __( '2Checkout fraud review in progress.', 'templify-recurring' ) );
							break;
						...
						case 'pass':
							...
							$initial_payment->add_note( __( '2Checkout fraud review passed.', 'templify-recurring' ) );
							break;
				...
				case 'INVOICE_STATUS_CHANGED' :
					...
					switch( $status ) {
						...
						case 'deposited' :
							...
							$initial_payment->update_status( 'complete' );
							$initial_payment->add_note( __( '2Checkout Invoice status set to deposited.', 'templify-recurring' ) );
							break;
				...
				case 'REFUND_ISSUED' :
					...
					if( isset( $_POST['item_list_amount_' . ( $cart_count + 1 ) ] ) && $_POST['item_list_amount_' . ( $cart_count + 1 ) ] < $payment->total ) {
						...
						$initial_payment->add_note( __( 'Payment refunded in 2Checkout.', 'templify-recurring' ) );
