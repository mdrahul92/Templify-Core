...
class EDD_Recurring_2Checkout_Onsite extends EDD_Recurring_2Checkout {
	...
	public function init() {
		...
		$this->friendly_name = __( '2Checkout OnSite', 'templify-recurring' );
	...
	public function validate_fields( $data, $posted ) {
		...
		if( empty( $this->credentials['tco_secret_word'] ) || empty( $this->credentials['tco_account_number'] ) ) {
			...
			edd_set_error( 'missing_api_keys', __( 'You must enter your account number and secret word in settings', 'templify-recurring' ) );

		}

		if( empty( $this->credentials['tco_private_key'] ) || empty( $this->credentials['tco_public_key'] ) ) {

			edd_set_error( 'missing_api_keys', __( 'You must enter your Private and Publishable API key in settings', 'templify-recurring' ) );
	...
	public function create_payment_profiles() {
		...
		if( empty( $_POST['token'] ) ) {
			edd_set_error( 'missing_card_token', __( 'Missing 2Checkout token, please try again or contact support if the issue persists.', 'templify-recurring' ) );
			return;
		}
