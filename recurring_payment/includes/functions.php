...
function edd_recurring_get_subscription_statuses() {
	return array(
		'pending'   => __( 'Pending', 'templify-recurring' ),
		'active'    => __( 'Active', 'templify-recurring' ),
		'cancelled' => __( 'Cancelled', 'templify-recurring' ),
		'expired'   => __( 'Expired', 'templify-recurring' ),
		'trialling' => __( 'Trialling', 'templify-recurring' ),
		'failing'   => __( 'Failing', 'templify-recurring' ),
		'completed' => __( 'Completed', 'templify-recurring' ),
	);
}
...
function edd_recurring_get_subscription_billing_text( $details, $recurring_amount = '' ) {
	...
	if ( empty( $details['times'] ) ) {
		...
		$output = sprintf( __( 'Billed%2$s once per %1$s until cancelled', 'templify-recurring' ), edd_recurring_get_frequency_label( $details['period'] ), $amount );
		if ( $details['trial_period'] && $details['trial_unit'] ) {
			$output = sprintf(
				...
				__( 'Billed%4$s once per %1$s until cancelled, after a %2$s %3$s free trial', 'templify-recurring' ),
...
function edd_recurring_get_frequency_label( $period, $count = 1 ) {
	...
	switch ( $period ) {
		case 'day':
			$frequency = _nx( 'day', 'days', $count, 'subscription term', 'templify-recurring' );
			break;
		case 'week':
			$frequency = _nx( 'week', 'weeks', $count, 'subscription term', 'templify-recurring' );
			break;
		case 'month':
			$frequency = _nx( 'month', 'months', $count, 'subscription term', 'templify-recurring' );
			break;
		case 'quarter':
			$frequency = _x( 'quarter', 'subscription term', 'templify-recurring' );
			break;
		case 'semi-year':
			$frequency = _x( 'six months', 'subscription term', 'templify-recurring' );
			break;
		case 'year':
			$frequency = _nx( 'year', 'years', $count, 'subscription term', 'templify-recurring' );
			break;
		default:
			$frequency = $period;
			break;
