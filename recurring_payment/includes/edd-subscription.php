...
class EDD_Subscription {
	...
	public function __get( $key ) {
		...
		if( method_exists( $this, 'get_' . $key ) ) {
			...
			return new WP_Error( 'edd-subscription-invalid-property', sprintf( __( 'Can\'t get property %s', 'templify-recurring' ), $key ) );
	...
	public function update( $args = array() ) {
		...
		if ( $ret ) {
			...
			if ( isset( $args['product_id'] ) && $current_product != $args['product_id'] ) {
				$this->add_note( sprintf( __( 'Product ID changed from %d to %d.', 'templify-recurring' ), $current_product, $args['product_id'] ) );
			}

			if ( isset( $args['price_id'] ) && ! is_null( $args['price_id'] ) && $current_price_id != $args['price_id'] ) {
				$this->add_note( sprintf( __( 'Price ID changed from %d to %d.', 'templify-recurring' ), $current_price_id, $args['price_id'] ) );
			}
		}
	...
	public function cancel() {
		...
		if ( $this->update( $args ) ) {
			...
			if ( is_user_logged_in() ) {
				...
				$user = __( 'gateway', 'templify-recurring' );
			...
			$note = sprintf( __( 'Subscription #%1$d cancelled by %2$s', 'templify-recurring' ), $this->id, $user );
			$this->add_note( $note );
	...
	public function retry() {
		...
		if ( ! $this->can_retry() ) {
			return new WP_Error( 'edd_recurring_not_failing', __( 'This subscription is not failing so cannot be retried.', 'templify-recurring' ) );
		}
		...
		if( ! $result ) {
			...
			$result = new WP_Error( 'edd_recurring_retry_failed', __( 'An error was encountered. Please check your merchant account logs.', 'templify-recurring' ) );
		}
		...
		if ( $gateway ) {
			...
			if( is_callable( array( $gateway, 'get_expiration' ) ) ) {
				...
				if( ! is_wp_error( $expiration ) && $this->get_expiration_time() < strtotime( $expiration, current_time( 'timestamp' ) ) ) {
					...
					$this->add_note( sprintf( __( 'Expiration synced with gateway and updated to %s', 'templify-recurring' ), $expiration ) );
	...
	public function get_status_label() {
		...
		switch ( $this->get_status() ) {
			case 'active':
				$status = __( 'Active', 'templify-recurring' );
				break;

			case 'cancelled':
				$status = __( 'Cancelled', 'templify-recurring' );
				break;

			case 'expired':
				$status = __( 'Expired', 'templify-recurring' );
				break;

			case 'pending':
				$status = __( 'Pending', 'templify-recurring' );
				break;

			case 'failing':
				$status = __( 'Failing', 'templify-recurring' );
				break;

			case 'trialling':
				$status = __( 'Trialling', 'templify-recurring' );
				break;

			case 'completed':
				$status = __( 'Completed', 'templify-recurring' );
				break;
	...
	protected function set_status( $new_status ) {
		...
		if( strtolower( $this->status ) !== strtolower( $old_status ) ) {
			$this->add_note( sprintf( __( 'Status changed from %s to %s by %s', 'templify-recurring' ), $old_status, $this->status, $user ) );
		}
