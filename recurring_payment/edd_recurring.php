...
final class EDD_Recurring {
	...
	private function load_textdomain() {
		...
		$locale = apply_filters( 'plugin_locale', get_locale(), 'templify-recurring' );
		$mofile = sprintf( '%1$s-%2$s.mo', 'templify-recurring', $locale );
		...
		if ( file_exists( $mofile_global ) ) {
			...
			load_textdomain( 'templify-recurring', $mofile_global );
		} elseif ( file_exists( $mofile_local ) ) {
			// Look in local /wp-content/plugins/edd-recurring/languages/ folder
			load_textdomain( 'templify-recurring', $mofile_local );
		} else {
			// Load the default language files
			load_plugin_textdomain( 'templify-recurring', false, $edd_lang_dir );
		}
	...
	public function register_post_statuses() {
		register_post_status( 'cancelled', array(
			'label'                     => _x( 'Cancelled', 'Cancelled payment status', 'templify-recurring' ),
			'public'                    => true,
			'exclude_from_search'       => false,
			'show_in_admin_all_list'    => false,
			'show_in_admin_status_list' => false,
			'label_count'               => _n_noop( 'Cancelled <span class="count">(%s)</span>', 'Cancelled <span class="count">(%s)</span>', 'templify-recurring' )
		) );
		register_post_status( 'edd_subscription', array(
			'label'                     => _x( 'Renewal', 'Subscription renewal payment status', 'templify-recurring' ),
			'public'                    => true,
			'exclude_from_search'       => false,
			'show_in_admin_all_list'    => true,
			'show_in_admin_status_list' => true,
			'label_count'               => _n_noop( 'Renewal <span class="count">(%s)</span>', 'Renewals <span class="count">(%s)</span>', 'templify-recurring' )
		) );
	}
	...
	public function subscriptions_list() {
		add_submenu_page(
			...
			__( 'Subscriptions', 'templify-recurring' ),
			__( 'Subscriptions', 'templify-recurring' ),
			'view_subscriptions',
			'edd-subscriptions',
	...
	public function register_edd_cancelled_status( $stati ) {
		$stati['edd_subscription'] = __( 'Renewal', 'templify-recurring' );
		return $stati;
	}
	...
	public function payments_view( $views ) {
		...
		$views['edd_subscription'] = sprintf(
			...
			__( 'Renewals', 'templify-recurring' ) . $subscription_count
		);
	...
	public function maybe_add_remove_fees() {
		...
		if ( $has_recurring && ( $fee_amount > 0 || $fee_amount < 0 ) ) {
			$args = array(
				'amount' => $fee_amount,
				'label'  => edd_get_option( 'recurring_signup_fee_label', __( 'Signup Fee', 'templify-recurring' ) ),
				'id'     => 'signup_fee',
				'type'   => 'fee'
			);
	...
	public function process_download( $download_id = 0, $email = '', $payment_id = 0, $args = array() ) {
		...
		wp_die(
			sprintf(
				...
				esc_html__( 'You must have an active subscription to %s in order to download this file.', 'templify-recurring' ),
				esc_html( get_the_title( $download_id ) )
			),
			esc_html__( 'Access Denied', 'templify-recurring' )
		);
	}
	...
	static function periods() {
		$periods = array(
			'day'       => _x( 'Daily', 'Billing period', 'templify-recurring' ),
			'week'      => _x( 'Weekly', 'Billing period', 'templify-recurring' ),
			'month'     => _x( 'Monthly', 'Billing period', 'templify-recurring' ),
			'quarter'   => _x( 'Quarterly', 'Billing period', 'templify-recurring' ),
			'semi-year' => _x( 'Semi-Yearly', 'Billing period', 'templify-recurring' ),
			'year'      => _x( 'Yearly', 'Billing period', 'templify-recurring' ),
		);
	...
	static function singular_periods() {
		$periods = array(
			'day'       => _x( 'Day(s)', 'Billing period', 'templify-recurring' ),
			'week'      => _x( 'Week(s)', 'Billing period', 'templify-recurring' ),
			'month'     => _x( 'Month(s)', 'Billing period', 'templify-recurring' ),
			'quarter'   => _x( 'Quarter(s)', 'Billing period', 'templify-recurring' ),
			'semi-year' => _x( 'Semi-Year(s)', 'Billing period', 'templify-recurring' ),
			'year'      => _x( 'Year(s)', 'Billing period', 'templify-recurring' ),
		);
	...
	public function enqueue_styles() {
		wp_register_style( 'templify-recurring', EDD_RECURRING_PLUGIN_URL . 'assets/css/styles.css', array(), EDD_RECURRING_VERSION );
		wp_enqueue_style( 'templify-recurring' );
	}
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'edd-frontend-recurring', 'edd_recurring_vars', array(
			'confirm_cancel' => __( 'Are you sure you want to cancel your subscription?', 'templify-recurring' ),
			...
			'trial_message'  => __( 'Your account will be automatically charged when the free trial is completed.', 'templify-recurring' ),
		) );
	...
	public function get_pretty_subscription_frequency( $period ) {
		...
		switch ( $period ) {
			case 'day' :
				$frequency = __( 'Daily', 'templify-recurring' );
				break;
			case 'week' :
				$frequency = __( 'Weekly', 'templify-recurring' );
				break;
			case 'month' :
				$frequency = __( 'Monthly', 'templify-recurring' );
				break;
			case 'quarter' :
				$frequency = __( 'Quarterly', 'templify-recurring' );
				break;
			case 'semi-year' :
				$frequency = __( 'Semi-Yearly', 'templify-recurring' );
				break;
			case 'year' :
				$frequency = __( 'Yearly', 'templify-recurring' );
				break;
			default :
	...
	public function get_pretty_singular_subscription_frequency( $period ) {
		...
		switch ( $period ) {
			case 'day' :
				$frequency = __( 'Day', 'templify-recurring' );
				break;
			case 'week' :
				$frequency = __( 'Week', 'templify-recurring' );
				break;
			case 'month' :
				$frequency = __( 'Month', 'templify-recurring' );
				break;
			case 'quarter' :
				$frequency = __( 'Quarter', 'templify-recurring' );
				break;
			case 'semi-year' :
				$frequency = __( 'Semi-Year', 'templify-recurring' );
				break;
			case 'year' :
				$frequency = __( 'Year', 'templify-recurring' );
				break;
			default :
		...
		if (
			...
			echo wpautop( wp_kses(
				sprintf(
					...
					__( '%1$sCredit card payments with Authorize.net are currently disabled.%2$s', 'templify-recurring' ),
					'<strong>',
					'</strong>'
				)
				. '<br />' .
				sprintf(
					...
					__( 'To continue accepting recurring credit card payments with Authorize.net please update the Authorize.net Payment Gateway extension to version %1$s2.0%2$s.', 'templify-recurring' ),
					'<code>',
					'</code>'
				),
				array(
	...
	<div class="notice notice-error">
		<p>
			...
			printf(
				...
				__( 'The Easy Digital Downloads - Recurring Payments plugin requires PHP version %1$s. You are running version %2$s. %3$sLearn more about updating PHP.%4$s', 'templify-recurring' ),
