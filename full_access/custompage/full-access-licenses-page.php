...
function edd_all_access_pass_page() {
	...
	echo esc_html( __( 'No All Access Pass defined', 'templify-full-access' ) );
}
...
add_filter( 'edd_settings_page_title', function( $page_title, $current_page, $is_single_view ) {
	if ( 'templify-full-access-pass' === $current_page ) {
		return __( 'All Access', 'templify-full-access' );
	}
...
function edd_all_access_display_pass_details( $payment_id, $download_id, $price_id ) {
	...
	if ( ! current_user_can( 'view_shop_reports' ) ) {
		edd_set_error( 'edd-no-access', __( 'You are not permitted to view this data.', 'templify-full-access' ) );
	}

	if ( 'invalid' === $all_access_pass->status ) {
		edd_set_error( 'edd-invalid-all-access-pass', __( 'Invalid All Access Pass Provided.', 'templify-full-access' ) );
	}

	?>
	<div class="wrap">
		<h2><?php esc_html_e( 'All Access Pass Details', 'templify-full-access' ); ?></h2>
		<?php
		if ( edd_get_errors() ) {
			?>
		...
		<div id="edd-item-card-wrapper">
			...
			<div class="info-wrapper item-section">
				...
				<form id="edit-all-access-pass" method="post" action="<?php echo esc_url( admin_url( 'edit.php?post_type=download&page=templify-full-access-pass&payment_id=' . $all_access_pass->payment_id . '&download_id=' . $all_access_pass->download_id . '&price_id=' . $all_access_pass->price_id ) ); ?>">
					...
					<div class="item-info">
						...
						$reactivation_notice =
							...
							// Translators: The %1$s represents the link to the manually create an order. %2$s is to open a link in a new tab.
								__( 'To be reactivated, it must be repurchased. You can also reactivate it by <a href="%1$s" %2$s>manually creating a payment.</a>', 'templify-full-access' ),
								esc_url( $add_new_url ),
								$new_tab
							);
						...
						if ( 'expired' === $all_access_pass->status ) {
							...
							echo wp_kses_post( '<strong>' . __( 'Note:', 'templify-full-access' ) . '</strong> ' . __( 'This All Access Pass has expired.', 'templify-full-access' ) . ' ' );
						...
						if ( 'upgraded' === $all_access_pass->status ) {
							echo '<p style="margin-top: 0;">';
							echo wp_kses_post( '<strong>' . __( 'Note:', 'templify-full-access' ) . '</strong> ' . __( 'This All Access Pass has been upgraded.', 'templify-full-access' ) . ' ' );
							echo wp_kses_post( $reactivation_notice );
							echo '</p>';
						...
						if ( 'renewed' === $all_access_pass->status ) {
							...
							if ( ! is_wp_error( $current_all_access_pass->payment_id ) ) {
								echo wp_kses_post( '<p style="margin-top: 0;"><strong>' . __( 'Note:', 'templify-full-access' ) . '</strong> ' . __( 'This All Access Pass has expired but was renewed by the customer. You can view the currently active All Access Pass', 'templify-full-access' ) . ' <a href="' . esc_url( admin_url( 'edit.php?post_type=download&page=templify-full-access-pass&payment_id=' . $current_all_access_pass->payment_id . '&download_id=' . $all_access_pass->download_id . '&price_id=' . $all_access_pass->price_id ) ) . '">' . __( 'here', 'templify-full-access' ) . '</a>.' );
							}
						}
						...
						if ( 'upcoming' === $all_access_pass->status ) {
							...
							if ( ! is_wp_error( $current_all_access_pass->payment_id ) ) {
								echo wp_kses_post( '<p style="margin-top: 0;"><strong>' . __( 'Note:', 'templify-full-access' ) . '</strong> ' . __( 'This All Access Pass is tied to a renewal payment and will "take over" when the current period ends. You can view the currently active All Access Pass', 'templify-full-access' ) . ' <a href="' . esc_url( admin_url( 'edit.php?post_type=download&page=templify-full-access-pass&payment_id=' . $current_all_access_pass->payment_id . '&download_id=' . $all_access_pass->download_id . '&price_id=' . $all_access_pass->price_id ) ) . '">' . __( 'here', 'templify-full-access' ) . '</a>.' );
							}
						...
						<table class="widefat striped">
							<tbody>
								<tr>
									<td class="row-title">
										<label for="tablecell"><?php esc_html_e( 'Customer:', 'templify-full-access' ); ?></label>
									</td>
									<td>
								...
								<tr>
									<td class="row-title">
										<label for="tablecell"><?php esc_html_e( 'Payment:', 'templify-full-access' ); ?></label>
									</td>
									<td>
										<?php echo esc_html( $payment_id ); ?> <a href="<?php echo esc_url( admin_url( 'edit.php?post_type=download&page=edd-payment-history&view=view-order-details&id=' . $payment_id ) ); ?>"><?php echo esc_html( __( '(View Order Details)', 'templify-full-access' ) ); ?></a>
									</td>
								</tr>
								<?php
								...
								if ( 'active' === $all_access_pass->status && ! empty( $all_access_pass->renewal_payment_ids ) ) {
									...
									<tr>
										<td class="row-title">
											<label for="tablecell"><?php esc_html_e( 'Early Renewal Payments:', 'templify-full-access' ); ?></label>
										</td>
										<td>
											<?php
								...
								<tr>
									<td class="row-title">
										<label for="tablecell"><?php esc_html_e( 'Product:', 'templify-full-access' ); ?></label>
									</td>
									<td>
								...
								<tr>
									<td class="row-title">
										<label for="tablecell"><?php esc_html_e( 'Downloaded Products:', 'templify-full-access' ); ?></label>
									</td>
									<td>
									<?php
									if ( function_exists( 'edd_get_admin_base_url' ) ) {
									...
									?>
										...
										<?php esc_html_e( 'View file download logs for this All Access pass.', 'templify-full-access' ); ?></a>
									</td>
								</tr>
								<?php } ?>
								<?php
								...
								<tr>
									<td class="row-title">
										<label for="tablecell"><?php esc_html_e( 'Start Date:', 'templify-full-access' ); ?></label>
									</td>
								...
								if ( 'upcoming' === $all_access_pass->status ) {
									...
									<tr>
										...
										if ( 'never' === $all_access_pass->expiration_time ) {
											echo esc_html( 'never' === $all_access_pass->expiration_time ? __( 'The current pass is set to never expire so this renewal payment will not be used.', 'templify-full-access' ) : edd_all_access_visible_date( 'M d, Y, g:i a', $renewal_start_time ) );
										} else {
									...
									<tr>
										<td class="row-title">
											<label for="tablecell"><?php esc_html_e( 'Expiration Date:', 'templify-full-access' ); ?></label>
										</td>
										<td>
										<?php
										// Expiration Date here is the expiration of the current pass plus its duration period.
										echo esc_html( 'never' === $all_access_pass->expiration_time ? __( 'Never Expires', 'templify-full-access' ) : edd_all_access_visible_date( 'M d, Y, g:i a', $renewal_expiration_time ) );
										?>
										</td>
									</tr>
										<?php
								...
								if ( 'upgraded' === $all_access_pass->status ) {
									...
									if ( $date ) {
										...
										<tr>
											<td class="row-title">
												<label for="tablecell"><?php esc_html_e( 'Upgraded Date:', 'templify-full-access' ); ?></label>
											</td>
								...
								if ( 'active' === $all_access_pass->status || 'expired' === $all_access_pass->status ) {
									...
									<tr>
										...
										<td>
										<?php
										echo esc_html( 'never' === $all_access_pass->expiration_time ? __( 'Never Expires', 'templify-full-access' ) : edd_all_access_visible_date( 'M d, Y, g:i a', $all_access_pass->expiration_time ) );
										?>
										</td>
									</tr>
								<?php } ?>
								...
								<tr>
									<td class="row-title">
										<label for="tablecell"><?php esc_html_e( 'Status:', 'templify-full-access' ); ?></label>
									</td>
									<td><?php echo esc_html( edd_all_access_get_status_label( $all_access_pass->status ) ); ?>
										...
										if ( 'upgraded' === $all_access_pass->status && ! is_wp_error( $upgraded_all_access_pass->payment_id ) ) {
											...
											?>
											<a href="<?php echo esc_url( admin_url( 'edit.php?post_type=download&page=templify-full-access-pass&payment_id=' . $upgraded_all_access_pass->payment->ID . '&download_id=' . $upgraded_all_access_pass->download_id . '&price_id=' . $upgraded_all_access_pass->price_id ) ); ?>"> <?php echo esc_html( __( '(View)', 'templify-full-access' ) ); ?></a>
										<?php } ?>
										<?php
										...
										if ( 'renewed' === $all_access_pass->status && ! is_wp_error( $current_all_access_pass->payment_id ) ) {
											...
											?>
											<a href="<?php echo esc_url( admin_url( 'edit.php?post_type=download&page=templify-full-access-pass&payment_id=' . $current_all_access_pass->payment_id . '&download_id=' . $all_access_pass->download_id . '&price_id=' . $all_access_pass->price_id ) ); ?>"> <?php echo esc_html( __( '(View Current Pass)', 'templify-full-access' ) ); ?></a>
										<?php } ?>
										<?php
										// If this All Access Pass is not yet active but will/should be once the current one expires (upcoming).
						...
						if ( 'active' === $all_access_pass->status || 'expired' === $all_access_pass->status ) {
							...
							<h2><?php echo esc_html( __( 'Settings at Time of Purchase', 'templify-full-access' ) ); ?></h2>
							<table class="widefat striped edd-aa-time-of-purchase-settings">
								<tbody>
									<tr>
										<td class="row-title">
											<label for="tablecell"><?php esc_html_e( 'Use these settings?', 'templify-full-access' ); ?></label>
										</td>
									...
									<tr>
										<td class="row-title">
											<label for="tablecell"><?php esc_html_e( 'All Access Duration:', 'templify-full-access' ); ?></label>
										</td>
										<?php
										if ( 'never' === $all_access_duration_unit ) {
											?>
											<td><?php echo esc_html( __( 'Never Expires', 'templify-full-access' ) ); ?></td>
											<?php
										} else {
									...
									if ( empty( $download_limit ) ) {
										$download_limit_per_period = __( 'Unlimited downloads per day', 'templify-full-access' );
									} else {
										$download_limit_per_period = $download_limit . ' ' . __( 'downloads per', 'templify-full-access' ) . ' ' . edd_all_access_download_limit_time_period_to_string( $download_limit_time_period );
									}
									?>
									<tr>
										<td class="row-title">
											<label for="tablecell"><?php esc_html_e( 'Download Limit:', 'templify-full-access' ); ?></label>
										</td>
										<td><?php echo esc_html( $download_limit_per_period ); ?></td>
									</tr>
									<tr>
										<td class="row-title">
											<label for="tablecell"><?php esc_html_e( 'All Access To:', 'templify-full-access' ); ?></label>
										</td>
										<td>
										<?php

										if ( empty( $all_access_categories ) || 'all' === $all_access_categories || ! is_array( $all_access_categories ) ) {
											echo esc_html( __( 'All Products', 'templify-full-access' ) );
										} else {
									...
									<tr>
										<td class="row-title">
											<label for="tablecell"><?php esc_html_e( 'Included Price Variations:', 'templify-full-access' ); ?></label>
										</td>
										<td>
											<?php
											if ( 0 === (int) $number_of_price_ids ) {
												echo esc_html( __( 'All price variations included.', 'templify-full-access' ) );
											} else {
												?>
												<ul>
													<?php
													for ( $included_price_id = 1; $included_price_id <= $number_of_price_ids; $included_price_id++ ) {
														...
														$variation_string = 1 === $included_price_id ? __( 'st Price Variation from each product', 'templify-full-access' ) : $variation_string;
														$variation_string = 2 === $included_price_id ? __( 'nd Price Variation from each product', 'templify-full-access' ) : $variation_string;
														$variation_string = 3 === $included_price_id ? __( 'rd Price Variation from each product', 'templify-full-access' ) : $variation_string;
							...
							<h2><?php echo esc_html( __( 'Customer-Specific Settings (overrides above settings)', 'templify-full-access' ) ); ?></h2>
							<table class="widefat striped">
								<tbody>
									<tr>
										<td class="row-title">
									...
									<tr>
										...
										?>
											...
											<span id="edd_all_access_unlimited_download_limit_note" style="display:none;"><?php echo esc_html( __( '(0 = Unlimited downloads per day)', 'templify-full-access' ) ); ?></span>
											<?php
									...
									<tr>
										...
										if ( empty( $all_access_categories ) || 'all' === $all_access_categories || ! is_array( $all_access_categories ) ) {
											$all_access_categories = array( 'all' => __( 'All Products', 'templify-full-access' ) );
										}
										...
										echo EDD()->html->select(
											array(
												...
												'multiple' => true,
												'show_option_all' => __( 'All Products', 'templify-full-access' ),
									...
									<tr>
										<td class="row-title">
											<label for="tablecell"><?php esc_html_e( 'Price Variations:', 'templify-full-access' ); ?></label>
										</td>
										<td>
											<?php
											echo esc_html( __( 'At most, included products have', 'templify-full-access' ) );
											?>
											<input type="number" name="edd_all_access_meta[number_of_price_ids]" id="edd_all_access_number_of_price_ids" value="<?php echo esc_attr( $number_of_price_ids ); ?>" min="0" style="width:40px;" /><?php echo esc_html( __( 'price variations.', 'templify-full-access' ) ); ?>
											<p><strong><?php echo esc_html( __( 'Included Price Variations:', 'templify-full-access' ) ); ?></strong></p>
											<span id="edd_all_access_included_price_ids_note" style="display:none;"><?php echo esc_html( __( 'Because this is set to 0, all price variations are included.', 'templify-full-access' ) ); ?></span>
											<ul id="edd_all_access_included_price_ids" style="display:none;">
												<?php
												for ( $price_id = 1; $price_id <= $number_of_price_ids; $price_id++ ) {

													$variation_string = __( 'th Price Variation from each product', 'templify-full-access' );
													$variation_string = 1 === $price_id ? __( 'st Price Variation from each product', 'templify-full-access' ) : $variation_string;
													$variation_string = 2 === $price_id ? __( 'nd Price Variation from each product', 'templify-full-access' ) : $variation_string;
													$variation_string = 3 === $price_id ? __( 'rd Price Variation from each product', 'templify-full-access' ) : $variation_string;
						...
						<div id="item-edit-actions" class="edit-item" style="display: block; margin-top:10px;">
							...
							<?php wp_nonce_field( 'templify-full-access-update', 'templify-full-access-update-nonce', false, true ); ?>
							<input type="submit" name="edd_update_all_access_pass" id="edd_update_all_access_pass" class="button button-primary" value="<?php echo esc_attr( 'Update All Access Pass', 'templify-full-access' ); ?>"/>
						</div>
					<?php } ?>
				</form>
			</div>
...
function edd_all_access_process_update() {
	...
	if ( ! isset( $_POST['templify-full-access-update-nonce'] ) ) {
		wp_die( __( 'Nonce verification failed', 'templify-full-access' ), __( 'Error', 'templify-full-access' ), array( 'response' => 403 ) );
	}

	if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['templify-full-access-update-nonce'] ) ), 'templify-full-access-update' ) ) {
		wp_die( __( 'Nonce verification failed', 'templify-full-access' ), __( 'Error', 'templify-full-access' ), array( 'response' => 403 ) );
	}

	if ( ! isset( $_POST['edd_all_access_meta_to_use'] ) ) {
		wp_die( __( 'Missing paramater', 'templify-full-access' ), __( 'Error', 'templify-full-access' ), array( 'response' => 403 ) );
	}

	$all_access_pass = edd_all_access_get_pass( absint( $_GET['payment_id'] ), absint( $_GET['download_id'] ), absint( $_GET['price_id'] ) );

	if ( 'invalid' === $all_access_pass->status ) {
		wp_die( __( 'That is not a valid All Access Pass.', 'templify-full-access' ), __( 'Error', 'templify-full-access' ), array( 'response' => 403 ) );
	}
	...
	wp_safe_redirect( admin_url( 'edit.php?post_type=download&page=templify-full-access-pass&payment_id=' . $all_access_pass->payment_id . '&download_id=' . $all_access_pass->download_id . '&price_id=' . $all_access_pass->price_id ) );
	exit;
