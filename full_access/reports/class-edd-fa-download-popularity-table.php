 ...
 * @since 1.1.2
 */
class EDD_FA_Download_Popularity_Table extends WP_List_Table {

	/**
	 * The EDD download ID where this AA pass originates.
	 *
	 * @var int $full_access_product_id
	 */
	private $full_access_product_id = null;

	/**
	 * The EDD variable price ID where this AA pass originates.
	 *
	 * @var int $full_access_price_id
	 */
	private $full_access_price_id = 0;
	...
	/**
	 * Set the values for the License in question
	 *
	 * @since 1.1.2
	 *
	 * @param int             $full_access_product_id The id of the AA product in question.
	 * @param int             $full_access_price_id   The variable price id of the AA product in question.
	 ...
	 *                                               results.
	 */
	public function set_filter_values( $full_access_product_id, $full_access_price_id, $aa_product_ids, $start_date, $end_date ) {
		$this->full_access_product_id = $full_access_product_id;
		$this->full_access_price_id   = $full_access_price_id;
	...
	public function get_columns() {
		return array(
			'downloaded_product_name' => __( 'Downloaded Product', 'templify-full-access' ),
			'number_of_downloads'     => __( 'Number of downloads', 'templify-full-access' ),
			'unique_passes_used'      => __( 'Unique Licensees Used', 'templify-full-access' ),
			'download_rate'           => __( 'Popularity Rate', 'templify-full-access' ),
		);
	}
	...
	public function prepare_items() {
		...
		if ( function_exists( 'edd_get_order' ) ) {
			...
			$this->set_filter_values(
				$product_id,
				$price_id,
				edd_full_access_get_full_access_downloads(),
		...
		$this->items = $this->get_popular_products( $this->full_access_product_id, $this->full_access_price_id, $this->start_date, $this->end_date );
	}
	...
	protected function query_popular_products() {
		...
		$product_id = $this->full_access_product_id;
		$price_id   = $this->full_access_price_id;

		if ( empty( $this->full_access_product_id ) || 'all' === $this->full_access_product_id || ! in_array( $this->full_access_product_id, $this->aa_product_ids ) ) {
		...
		$popular_products = $wpdb->get_results( $wpdb->prepare(
			...
			WHERE meta_key = '_edd_log_full_access_pass_id'
	 ...
	 *
	 * @param int    $full_access_product_id
	 * @param int    $full_access_price_id
	 ...
	 * @return array
	 */
	protected function query_popular_products_29( $full_access_product_id, $full_access_price_id, $start_date, $end_date ) {
		global $wpdb;

		/**
		 * Here's an example of the raw query which powers this:
		 * SELECT wp_38_posts.post_parent, count(wp_38_posts.post_parent)  from wp_38_posts LEFT JOIN wp_38_postmeta ON wp_38_postmeta.post_id = wp_38_posts.ID WHERE wp_38_postmeta.meta_key = '_edd_log_full_access_pass_id' AND wp_38_postmeta.meta_value LIKE '%_40_%' group by wp_38_posts.post_parent;
		 */
		...
		$total_unique_passes_used = $wpdb->get_results( // phpcs:ignore
			$wpdb->prepare(
				"SELECT count(distinct $post_meta_meta_value) as unique_passes_used from $posts_table LEFT JOIN $post_meta_table ON $post_meta_post_id = $posts_post_id WHERE $post_meta_meta_key = %s AND $post_meta_meta_value LIKE '%_{$full_access_product_id}_%' AND $posts_post_date_gmt > %s AND $posts_post_date_gmt < %s;", // phpcs:ignore
				array( '_edd_log_full_access_pass_id', $start_date_mysql, $end_date_mysql )
			),
			ARRAY_A
		);
	 ...
	 *
	 * @param int      $full_access_product_id The id of the EDD Download which is an Full Access License, and for which we want to know which products were downloaded.
	 * @param int      $full_access_price_id The variable price id of the EDD Download which is an Full Access License, and for which we want to know which products were downloaded.
	 ...
	 * @since 1.1.2
	 */
	public function get_popular_products( $full_access_product_id, $full_access_price_id, $start_date, $end_date ) {

		if ( function_exists( 'edd_get_order' ) ) {
			$query_results = $this->query_popular_products();
		} else {
			$query_results = $this->query_popular_products_29( $full_access_product_id, $full_access_price_id, $start_date, $end_date );
		}
	...
	public function column_downloaded_product_name( $item ) {
		...
		$actions   = array(
			'view' => '<a href="' . esc_url( get_permalink( $item['downloaded_product_id'] ) ) . '">' . esc_html__( 'View', 'templify-full-access' ) . '</a>',
		);
		if ( current_user_can( 'edit_products' ) ) {
			$actions['edit'] = '<a href="' . esc_url( $edit_link ) . '">' . esc_html__( 'Edit', 'templify-full-access' ) . '</a>';
		}
	...
	public function display_tablenav( $which ) {
		...
		<div id="edd-payment-filters" style="overflow:initial;">
			<span><strong><?php echo esc_html( __( 'Full Access Product:', 'templify-full-access' ) ); ?></strong></span>
			<form action="">
				<input type="hidden" name="view" value="edd_fa_popular_products" />
				...
				<span>
					...
					echo EDD()->html->select( // phpcs:ignore
						array(
							...
							'name'             => 'edd_fa_product_id',
							'selected'         => esc_attr( $this->full_access_product_id ),
							'id'               => 'edd_full_access_products',
							'class'            => 'edd_full_access_products',
							'chosen'           => true,
							'placeholder'      => esc_attr( __( 'Type to search Full Access Products', 'templify-full-access' ) ),
							'multiple'         => false,
				...
				<span id="edd-payment-date-filters">
					<span>
						<label for="start-date"><?php echo esc_html( __( 'Start Date:', 'templify-full-access' ) ); ?></label>
					...
					<span>
						<label for="end-date"><?php echo esc_html( __( 'End Date:', 'templify-full-access' ) ); ?></label>
				...
				<span id="edd-payment-after-core-filters">
					<input type="submit" class="button-secondary" value="<?php echo esc_attr( __( 'Apply', 'templify-full-access' ) ); ?>"/>
				</span>
			</form>
		</div>
			<?php
	...
	public function no_items() {
		$message = __( 'No items found.', 'templify-full-access' );

		if ( empty( $this->full_access_product_id ) || 'all' === $this->full_access_product_id ) {
			$message = __( 'Please select an Full Access License.', 'templify-full-access' );
		} elseif ( ! in_array( $this->full_access_product_id, $this->aa_product_ids ) ) {
			$message = __( 'The selected product is not an Full Access License.', 'templify-full-access' );
		}
