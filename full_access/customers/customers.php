 ...
 */
function edd_full_access_customer_tab( $tabs ) {
	...
	$tabs['all-access-passes'] = array(
		...
		'title'    => __( 'All Access Passes', 'templify-full-access' ),
	);

	return $tabs;
}

add_filter( 'edd_customer_tabs', 'edd_full_access_customer_tab', 10, 1 );
...
function edd_all_access_customer_view( $customer ) {
	...
	<div id="customer-tables-wrapper" class="customer-section">
		...
		?>
		<h3><?php esc_html_e( 'All Access Passes', 'templify-full-access' ); ?></h3>
		<table class="wp-list-table widefat striped downloads">
			...
			if ( is_array( $customer_all_access_passes ) ) {
				...
				foreach ( $customer_all_access_passes as $all_access_pass ) {
					...
					<tr>
						...
						<td>
							<a href="<?php echo esc_url( admin_url( 'edit.php?post_type=download&page=templify-full-access-pass&payment_id=' . $all_access_pass->payment->ID . '&download_id=' . $all_access_pass->download_id . '&price_id=' . $all_access_pass->price_id ) ); ?>"><?php esc_html_e( 'View Details', 'templify-full-access' ); ?></a>
					</tr>
					<?php
				}
			}
			...
			if ( ! $at_least_one_pass_to_show ) {
				?>
				<tr>
					<td colspan="5"><?php esc_html_e( 'No All Access Passes Found', 'templify-full-access' ); ?></td>
				</tr>
			<?php } ?>
			</tbody>
		</table>
		...
		if ( class_exists( 'EDD_Dev_Tools' ) ) {
			?>
			<div class="templify-full-access-dev-tools-area" style="background-color:#fffbb4; padding:10px;">'
				<h2><?php echo esc_html( __( 'Customer All Access Passes Array:', 'templify-full-access' ) ); ?></h2>
